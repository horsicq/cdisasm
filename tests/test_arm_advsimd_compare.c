...
#define COMPARE_MASK UINT32_C(0xbf20fc00)
...
static const compare_operation operations[5] = {
    { UINT32_C(0x0e203400), CDISASM_ARM_NAME_CMGT,
      UINT16_C(6120), "cmgt" },
    { UINT32_C(0x0e203c00), CDISASM_ARM_NAME_CMGE,
      UINT16_C(6121), "cmge" },
    { UINT32_C(0x2e203400), CDISASM_ARM_NAME_CMHI,
      UINT16_C(6162), "cmhi" },
    { UINT32_C(0x2e203c00), CDISASM_ARM_NAME_CMHS,
      UINT16_C(6163), "cmhs" },
    { UINT32_C(0x2e208c00), CDISASM_ARM_NAME_CMEQ,
      UINT16_C(6173), "cmeq" }
};

_Static_assert(CDISASM_ARM_NAME_CMGT == UINT16_C(676),
               "CMGT mnemonic ID changed");
_Static_assert(CDISASM_ARM_NAME_CMGE == UINT16_C(675),
               "CMGE mnemonic ID changed");
_Static_assert(CDISASM_ARM_NAME_CMHI == UINT16_C(677),
               "CMHI mnemonic ID changed");
_Static_assert(CDISASM_ARM_NAME_CMHS == UINT16_C(678),
               "CMHS mnemonic ID changed");
_Static_assert(CDISASM_ARM_NAME_CMEQ == UINT16_C(674),
               "CMEQ mnemonic ID changed");
_Static_assert(CDISASM_ARM_FORM_AARCHMRS_2026_03_LAST >= UINT16_C(6173),
               "Advanced SIMD compare form IDs unavailable");
...
static void test_exhaustive_envelopes(void)
{
    ...
    for (operation = 0u; operation < 5u; ++operation) {
        ...
        for (q = 0u; q < 2u; ++q) {
            EXPECT(partitions[operation][q][0]
                == (q != 0u ? UINT32_C(131072) : UINT32_C(98304)));
            EXPECT(partitions[operation][q][1]
                == (q != 0u ? UINT32_C(0) : UINT32_C(32768)));
        }
    }
    EXPECT(allocated == UINT32_C(1146880));
    EXPECT(reserved == UINT32_C(163840));
}
...
static int is_compare_form(cdisasm_arm_form_id form_id)
{
...
static void test_profiles_and_neighbors(void)
{
    ...
    for (operation = 0u; operation < 5u; ++operation) {
        ...
        for (bit = 0u; bit < 32u; ++bit) {
            ...
            if ((COMPARE_MASK & (UINT32_C(1) << bit)) == 0u) {
                continue;
            }
            memset(&instruction, 0xa5, sizeof(instruction));
...

    {
        static const uint32_t sibling_words[] = {
            ...
            UINT32_C(0x5ee03400), /* scalar CMGT */
            UINT32_C(0x7ee03c00), /* scalar CMHS */
            UINT32_C(0x0e208800), /* vector CMGT #0 */
            UINT32_C(0x2e208800), /* vector CMGE #0 */
            UINT32_C(0x2e209800)  /* vector CMEQ #0 */
        };
        size_t index;
        ...
        for (index = 0u;
            ...
            (void)decode_word(word, CDISASM_ARM_CPU_ANY,
                CDISASM_ARM_MODE_A64, 4u,
                CDISASM_ARM_DECODE_OPTION_NONE, &instruction);
            EXPECT(!is_compare_form(instruction.form_id));
        }
    }
}
...
static void test_transport_and_boundaries(void)
{
    ...
    for (operation = 0u; operation < 5u; ++operation) {
        ...
        memset(&other, 0xa5, sizeof(other));
        EXPECT(decode_word(make_word(operation, 0u, 3u,
...
static void test_formatter_and_collisions(void)
{
    ...
    expect_format(0u, 0u, 0u, "cmgt v7.8b, v13.8b, v29.8b");
    expect_format(0u, 1u, 1u, "cmgt v7.8h, v13.8h, v29.8h");
    expect_format(1u, 0u, 2u, "cmge v7.2s, v13.2s, v29.2s");
    expect_format(2u, 1u, 0u, "cmhi v7.16b, v13.16b, v29.16b");
    expect_format(3u, 0u, 1u, "cmhs v7.4h, v13.4h, v29.4h");
    expect_format(4u, 1u, 2u, "cmeq v7.4s, v13.4s, v29.4s");
    expect_format(4u, 1u, 3u, "cmeq v7.2d, v13.2d, v29.2d");
    ...
    EXPECT(cdisasm_arm_format(&instruction,
        ...
        text, sizeof(text))
        == strlen("CMGT v31.16b, v30.16b, v29.16b"));
    EXPECT(strcmp(text, "CMGT v31.16b, v30.16b, v29.16b") == 0);
...
#define REJECT_MUTATION(statement)                                          \
    ...
    REJECT_MUTATION(forged.form_id = UINT16_C(6121));
    REJECT_MUTATION(forged.name_id = CDISASM_ARM_NAME_CMGE);
    REJECT_MUTATION(forged.raw_instruction = make_word(
        2u, 1u, 0u, 29u, 30u, 31u));
    REJECT_MUTATION(forged.raw_instruction = make_word(
        0u, 0u, 3u, 29u, 30u, 31u));
    ...
    memset(&forged, 0, sizeof(forged));
    forged.name_id = CDISASM_ARM_NAME_CMGT;
    forged.raw_instruction = UINT32_C(0xd503201f);
    forged.opcode_size = 4u;
    forged.isa_id = CDISASM_ARM_ISA_A64;
    forged.condition = CDISASM_ARM_CONDITION_AL;
...
int main(void)
{
    ...
    if (failures != 0) {
        fprintf(stderr, "%d Advanced SIMD compare test(s) failed\n",
                failures);
        return 1;
    }
    printf("ARM Advanced SIMD compare tests passed "
           "(USE_EXTRA_OPCODES=%d, allocated=1146880, reserved=163840)\n",
           USE_EXTRA_OPCODES);
    return 0;
}
