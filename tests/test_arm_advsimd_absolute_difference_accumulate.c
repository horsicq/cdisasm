...
static const accumulate_operation operations[2] = {
    { UINT32_C(0x0e207c00), CDISASM_ARM_NAME_SABA,
      UINT16_C(6129), "saba" },
    { UINT32_C(0x2e207c00), CDISASM_ARM_NAME_UABA,
      UINT16_C(6171), "uaba" }
};

_Static_assert(CDISASM_ARM_NAME_SABA == UINT16_C(1297),
               "SABA mnemonic ID changed");
_Static_assert(CDISASM_ARM_NAME_UABA == UINT16_C(1728),
               "UABA mnemonic ID changed");
_Static_assert(CDISASM_ARM_FORM_AARCHMRS_2026_03_LAST >= UINT16_C(6171),
               "Advanced SIMD absolute-difference-accumulate forms unavailable");
...
static int is_fixed_accumulate_form(cdisasm_arm_form_id form_id)
{
...
static void test_profiles_and_neighbors(void)
{
    static const uint32_t sibling_words[] = {
        UINT32_C(0x4ebd75a7), /* fixed-vector SABD */
        UINT32_C(0x6ebd75a7), /* fixed-vector UABD */
        UINT32_C(0x4ebd51a7), /* SABAL2 */
        UINT32_C(0x6ebd51a7), /* UABAL2 */
        UINT32_C(0x4500f820), /* SVE SABA */
        UINT32_C(0x4500fc20)  /* SVE UABA */
    };
    unsigned operation;
    size_t index;
    ...
    for (index = 0u;
        ...
        (void)decode_word(sibling_words[index], CDISASM_ARM_CPU_ANY,
            CDISASM_ARM_MODE_A64, 4u,
            CDISASM_ARM_DECODE_OPTION_NONE, &instruction);
        EXPECT(!is_fixed_accumulate_form(instruction.form_id));
    }
}
...
static void test_transport_and_status_precedence(void)
{
    ...
    (void)decode_word(make_word(1u, 1u, 2u, 29u, 13u, 7u),
        ...
        CDISASM_ARM_DECODE_OPTION_NONE, &instruction);
    EXPECT(instruction.last_error_id != CDISASM_STATUS_OK
        || !is_fixed_accumulate_form(instruction.form_id));
}
...
static void test_formatter_and_schema(void)
{
    ...
    expect_format(UINT32_C(0x4500f820),
                  "saba z0.b, z1.b, z0.b");
    expect_format(UINT32_C(0x4500fc20),
                  "uaba z0.b, z1.b, z0.b");
    ...
    EXPECT(cdisasm_arm_format(&instruction,
        ...
        text, sizeof(text))
        == strlen("SABA v31.4s, v30.4s, v29.4s"));
    EXPECT(strcmp(text, "SABA v31.4s, v30.4s, v29.4s") == 0);
...
#define REJECT_MUTATION(statement)                                          \
    ...
    REJECT_MUTATION(forged.form_id = UINT16_C(6171));
    REJECT_MUTATION(forged.name_id = CDISASM_ARM_NAME_UABA);
    ...
    memset(&forged, 0, sizeof(forged));
    forged.name_id = CDISASM_ARM_NAME_SABA;
    forged.raw_instruction = UINT32_C(0xd503201f);
    forged.opcode_size = 4u;
    forged.isa_id = CDISASM_ARM_ISA_A64;
    forged.condition = CDISASM_ARM_CONDITION_AL;
    ...
    memset(&instruction, 0xa5, sizeof(instruction));
    EXPECT(decode_word(UINT32_C(0x4500f820), CDISASM_ARM_CPU_ANY,
        CDISASM_ARM_MODE_A64, 4u, CDISASM_ARM_DECODE_OPTION_NONE,
        &instruction) == 4u);
    REJECT_MUTATION(forged.form_id = UINT16_C(6129));
    REJECT_MUTATION(forged.raw_instruction = make_word(
        0u, 1u, 0u, 2u, 1u, 0u));
    REJECT_MUTATION(forged.name_id = CDISASM_ARM_NAME_UABA);
    REJECT_MUTATION(forged.instruction_flags &=
...
int main(void)
{
    ...
    if (failures != 0) {
        fprintf(stderr,
                "%d Advanced SIMD SABA/UABA test(s) failed\n",
                failures);
        return 1;
    }
    printf("ARM Advanced SIMD SABA/UABA tests passed "
