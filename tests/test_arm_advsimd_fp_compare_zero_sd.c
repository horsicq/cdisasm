...
static const struct operation operations[5] = {
    { UINT32_C(0x0ea0c800), CDISASM_ARM_NAME_FCMGT, 6027 },
    { UINT32_C(0x0ea0d800), CDISASM_ARM_NAME_FCMEQ, 6028 },
    { UINT32_C(0x0ea0e800), CDISASM_ARM_NAME_FCMLT, 6029 },
    { UINT32_C(0x2ea0c800), CDISASM_ARM_NAME_FCMGE, 6063 },
    { UINT32_C(0x2ea0d800), CDISASM_ARM_NAME_FCMLE, 6064 }
};
    ...
    cdisasm_arm_instruction *instruction)
{
    ...
    return cdisasm_arm_decode(cpu, CDISASM_ARM_MODE_A64, bytes, 4u,
        UINT64_C(0x0ea0c800), CDISASM_ARM_DECODE_OPTION_NONE, instruction);
}
        ...
        for (arrangement = 0u; arrangement < 3u; ++arrangement) {
            ...
            for (rn = 0u; rn < 32u; ++rn) {
                for (rd = 0u; rd < 32u; ++rd) {
                    ...
                    EXPECT(instruction.operand[2].size == 1u);
...
static void test_reserved_and_profile(void)
{
    ...
    }
    {
        ...
        memset(&instruction, 0xa5, sizeof(instruction));
#if USE_EXTRA_OPCODES
        EXPECT(decode_word(UINT32_C(0x0ea0d820), CDISASM_ARM_CPU_CORTEX_A53,
            &instruction) == 4u);
#else
        EXPECT(decode_word(UINT32_C(0x0ea0d820), CDISASM_ARM_CPU_CORTEX_A53,
            &instruction) == 0u);
...
int main(void)
{
    ...
    puts("Advanced SIMD S/D floating compare-zero tests passed");
    return 0;
}
