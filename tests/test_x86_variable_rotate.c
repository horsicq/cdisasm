...
_Static_assert(APX_B4_U1_CASES + APX_U0_NOSIB_CASES
    ...
    "APX variable-rotate matrix total changed");

static const rotate_case cases[] = {
    {0x15, 0, CDISASM_X86_NAME_VPROLVD, "vprolvd"},
    {0x15, 1, CDISASM_X86_NAME_VPROLVQ, "vprolvq"},
    {0x14, 0, CDISASM_X86_NAME_VPRORVD, "vprorvd"},
    {0x14, 1, CDISASM_X86_NAME_VPRORVQ, "vprorvq"}
};

_Static_assert(CDISASM_X86_NAME_VPROLVD == UINT16_C(1046),
    "variable-rotate catalog start changed");
_Static_assert(CDISASM_X86_NAME_VPRORVQ == UINT16_C(1049),
    "variable-rotate catalog end changed");
_Static_assert(CDISASM_X86_NAME_COUNT == CDISASM_X86_NAME_LAST + UINT16_C(1),
    "variable-rotate public catalog is incomplete");
_Static_assert(CDISASM_NAME_VPROLVD == CDISASM_X86_NAME_VPROLVD
        && CDISASM_NAME_VPROLVQ == CDISASM_X86_NAME_VPROLVQ
        && CDISASM_NAME_VPRORVD == CDISASM_X86_NAME_VPRORVD
        && CDISASM_NAME_VPRORVQ == CDISASM_X86_NAME_VPRORVQ,
    "legacy variable-rotate aliases changed");
            ...
            for (length = 0; length != 3; ++length) {
                ...
                for (source = SOURCE_REGISTER;
                    ...
                    for (mask = MASK_NONE;
                        ...
                        (void)vector_bits;
                        (void)element_bits;
                        expect_error("extra-opcodes OFF EVEX variable rotate",
...
#else
                (void)memory_bits;
                expect_error("extra-opcodes OFF rotate disp8",
...
static void test_feature_routes(void)
{
    ...
    expect_error("variable-rotate runtime AVX-512 gate",
        CDISASM_CPU_SKYLAKE_SP, CDISASM_MODE_64,
        evex, sizeof(evex), CDISASM_X86_DECODE_FLAG_BASE,
        CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
    expect_error("variable-rotate CPU AVX-512 gate",
        CDISASM_CPU_HASWELL, CDISASM_MODE_64,
        evex, sizeof(evex), CDISASM_X86_DECODE_FLAG_AVX512,
        CDISASM_STATUS_INVALID_INSTRUCTION);
    expect_error("legacy profile rejects rotate AVX10 route",
        CDISASM_CPU_SKYLAKE_SP, CDISASM_MODE_64,
        evex, sizeof(evex), CDISASM_X86_DECODE_FLAG_AVX10,
        CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
    expect_error("AVX10 profile rejects rotate AVX-512 route",
    ...
    EXPECT(instruction.name_id == CDISASM_X86_NAME_VPROLVD);
    EXPECT(cdisasm_instruction_has_x86_group(
        &instruction, CDISASM_X86_GROUP_AVX10_1));
    EXPECT(!cdisasm_instruction_has_x86_group(
        &instruction, CDISASM_X86_GROUP_AVX512F));
...
#else
    expect_error("extra-opcodes OFF rotate feature route",
        CDISASM_CPU_X86, CDISASM_MODE_64,
        evex, sizeof(evex), CDISASM_X86_DECODE_FLAG_BASE,
        CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
#endif
...
static void test_apx_b4(void)
{
        ...
        for (length = 0; length != 3; ++length) {
            ...
            for (source = SOURCE_REGISTER;
                ...
                for (mask = MASK_NONE;
                    ...
                    (void)vector_bits;
                    (void)element_bits;
                    expect_error("extra-opcodes OFF APX B4 rotate",
                        CDISASM_CPU_X86, CDISASM_MODE_64,
                        code, sizeof(code), CDISASM_X86_DECODE_FLAG_BASE,
                        CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
#endif
...

    {
        static const uint8_t apx_register[] = {
            0x62, 0xfa, 0x6d, 0x4a, 0x15, 0xcb
        };
#if USE_EXTRA_OPCODES
        uint32_t decoded_size;
        cdisasm_instruction instruction;

        expect_error("APX rotate runtime gate",
            CDISASM_CPU_APX, CDISASM_MODE_64,
            apx_register, sizeof(apx_register),
            CDISASM_X86_DECODE_FLAG_AVX10,
            CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
        expect_error("APX rotate CPU gate",
            CDISASM_CPU_AVX10, CDISASM_MODE_64,
            apx_register, sizeof(apx_register),
            CDISASM_X86_DECODE_FLAG_AVX10
                | CDISASM_X86_DECODE_FLAG_APX,
            CDISASM_STATUS_INVALID_INSTRUCTION);
        expect_error("APX rotate mode gate",
        ...
        EXPECT(cdisasm_instruction_has_x86_group(
            &instruction, CDISASM_X86_GROUP_APX_F));
#else
        expect_error("extra-opcodes OFF APX rotate gate control",
            CDISASM_CPU_X86, CDISASM_MODE_64,
            apx_register, sizeof(apx_register),
            CDISASM_X86_DECODE_FLAG_BASE,
            CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
...
static void test_apx_u0_memory(void)
{
            ...
            for (b4 = 0; b4 != 2; ++b4) {
                ...
                for (source = SOURCE_FULL_MEMORY;
                    ...
                    for (mask = MASK_NONE;
                        ...
                        (void)vector_bits;
                        (void)element_bits;
                        expect_error("extra-opcodes OFF APX U0 rotate",
                            CDISASM_CPU_X86, CDISASM_MODE_64,
                            code, sizeof(code),
                            CDISASM_X86_DECODE_FLAG_BASE,
                            CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
#endif
                        {
                            ...
                            memcpy(x4_code, code, sizeof(code));
                            x4_code[5] = UINT8_C(0x0c);
...
#else
                            expect_error(
                                "extra-opcodes OFF APX U0/X4 rotate",
                                CDISASM_CPU_X86, CDISASM_MODE_64,
                                x4_code, sizeof(x4_code),
                                CDISASM_X86_DECODE_FLAG_BASE,
                                CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
    ...
    for (case_index = 0;
        ...
        for (address_index = 0;
            ...
            const size_t code_size = address_index == 0 ? 6u : 7u;
...
#else
            expect_error("extra-opcodes OFF APX X4 rotate",
                CDISASM_CPU_X86, CDISASM_MODE_64,
...

    {
        static const uint8_t u0_memory[] = {
            0x62, 0xf2, 0x69, 0x0a, 0x15, 0x08
        };
        static const uint8_t u0_register[] = {
            0x62, 0xf2, 0x69, 0x0a, 0x15, 0xcb
        };

        expect_error("APX rotate U0 register stays reserved",
            CDISASM_CPU_X86, CDISASM_MODE_64,
            u0_register, sizeof(u0_register),
            CDISASM_X86_DECODE_FLAG_BASE,
            CDISASM_STATUS_INVALID_INSTRUCTION);
        expect_error("APX rotate U0 unavailable outside mode64",
            CDISASM_CPU_X86, CDISASM_MODE_32,
            u0_memory, sizeof(u0_memory),
            CDISASM_X86_DECODE_FLAG_BASE,
            CDISASM_STATUS_INVALID_INSTRUCTION);
#if USE_EXTRA_OPCODES
        expect_error("APX rotate U0 runtime gate",
            CDISASM_CPU_APX, CDISASM_MODE_64,
            u0_memory, sizeof(u0_memory),
            CDISASM_X86_DECODE_FLAG_AVX10,
            CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
        expect_error("APX rotate U0 CPU gate",
            CDISASM_CPU_AVX10, CDISASM_MODE_64,
            u0_memory, sizeof(u0_memory),
            CDISASM_X86_DECODE_FLAG_AVX10
                | CDISASM_X86_DECODE_FLAG_APX,
            ...
            for (source = SOURCE_REGISTER;
                ...
                for (mask = MASK_NONE;
                    ...
                    code[3] = (uint8_t)((code[3] & UINT8_C(0x9f))
                        | UINT8_C(0x60));
                    expect_error("reserved rotate LL=3",
                        CDISASM_CPU_X86, modes[mode_index],
                        code, sizeof(code), CDISASM_X86_DECODE_FLAG_BASE,
                        CDISASM_STATUS_INVALID_INSTRUCTION);
                    ++reserved_cases;
...

            {
                ...
                for (length = 0; length != 3; ++length) {
                    ...
                    for (mask = MASK_NONE;
                        ...
                        make_encoding(&cases[case_index], length,
                            SOURCE_REGISTER, mask, register_b);
                        memcpy(u0_register, register_b,
                            sizeof(u0_register));
                        register_b[3] |= UINT8_C(0x10);
                        u0_register[2] &= UINT8_C(0xfb);
                        expect_error("reserved rotate register EVEX.b",
                            CDISASM_CPU_X86, modes[mode_index],
                            register_b, sizeof(register_b),
                            CDISASM_X86_DECODE_FLAG_BASE,
                            CDISASM_STATUS_INVALID_INSTRUCTION);
                        expect_error("reserved rotate U0 register",
                            CDISASM_CPU_X86, modes[mode_index],
                            u0_register, sizeof(u0_register),
                            CDISASM_X86_DECODE_FLAG_BASE,
                            CDISASM_STATUS_INVALID_INSTRUCTION);
                    ...
                    for (source = SOURCE_REGISTER;
                        ...
                        make_encoding(&cases[case_index], length,
                            source, MASK_NONE, zero_without_mask);
                        zero_without_mask[3] |= UINT8_C(0x80);
                        expect_error("reserved rotate zero without mask",
                            CDISASM_CPU_X86, modes[mode_index],
                            zero_without_mask,
...
int main(void)
{
    ...
    if (failures != 0) {
        fprintf(stderr, "%d x86 variable-rotate test(s) failed\n", failures);
        return 1;
    }
    printf("x86 variable-rotate tests passed (extra=%d, format=%d)\n",
