...
struct operation {
    ...
    cdisasm_arm_form_id store_form, load_form;
};

static const struct operation operations[4] = {
    { UINT32_C(0x0000), 4u, CDISASM_ARM_NAME_ST4,
      CDISASM_ARM_NAME_LD4, 4573, 4580 },
    { UINT32_C(0x2000), 4u, CDISASM_ARM_NAME_ST1,
      CDISASM_ARM_NAME_LD1, 4574, 4581 },
    { UINT32_C(0x4000), 3u, CDISASM_ARM_NAME_ST3,
      CDISASM_ARM_NAME_LD3, 4575, 4582 },
    { UINT32_C(0x6000), 3u, CDISASM_ARM_NAME_ST1,
      CDISASM_ARM_NAME_LD1, 4576, 4583 }
};
    ...
    cdisasm_arm_instruction *instruction)
{
    ...
    return cdisasm_arm_decode(cpu, CDISASM_ARM_MODE_A64, bytes, 4u,
        UINT64_C(0x0c000000), CDISASM_ARM_DECODE_OPTION_NONE, instruction);
}
    ...
    unsigned rn, unsigned rt)
{
...
#endif
    uint32_t word = UINT32_C(0x0c000000) | operations[op].opcode
        | (load << 22) | (q << 30) | (size << 10) | (rn << 5) | rt;
    ...
    EXPECT(instruction.form_id == (load ? operations[op].load_form
                                       : operations[op].store_form));
    ...
    EXPECT(instruction.operand[1].size == register_size * operations[op].count);
...
int main(void)
{
        ...
        for (load = 0u; load < 2u; ++load)
            for (q = 0u; q < 2u; ++q)
                for (size = 0u; size < 4u; ++size)
                    for (rn = 0u; rn < 32u; ++rn)
                        for (rt = 0u; rt < 32u; ++rt)
                            check(op, load, q, size, rn, rt);
    {
        cdisasm_arm_instruction instruction;
        memset(&instruction, 0xa5, sizeof(instruction));
#if USE_EXTRA_OPCODES
        EXPECT(decode_word(UINT32_C(0x4c402ffc),
            CDISASM_ARM_CPU_CORTEX_A53, &instruction) == 4u);
        EXPECT(instruction.operand[0].reg == CDISASM_ARM_REG_V28);
    ...
    puts("Advanced SIMD 3/4-register structure tests passed");
    return 0;
}
