...
_Static_assert(CDISASM_X86_NAME_VMOVSS == UINT16_C(1792),
    "VMOVSS name ID changed");
    ...
    int zero)
{
    ...
    const int memory_destination =
        form_id == UINT16_C(5940) || form_id == UINT16_C(5939);
    const int memory_source =
        form_id == UINT16_C(5941) || form_id == UINT16_C(5944);
    const unsigned int expected_operands =
        memory_destination || memory_source ? 2u : 3u;

    EXPECT(decoded_size != 0u);
    EXPECT(instruction->last_error_id == CDISASM_STATUS_OK);
    EXPECT(instruction->name_id == CDISASM_X86_NAME_VMOVSS);
...
static void test_representative_forms_and_legacy(void)
{
    static const uint8_t modern[][7] = {
        {0xc5,0xfa,0x10,0x00,0,0,0},
        {0xc5,0xfa,0x11,0x00,0,0,0},
        {0xc5,0xea,0x10,0xcb,0,0,0},
        {0xc5,0xea,0x11,0xcb,0,0,0},
        {0x62,0xf1,0x7e,0x08,0x10,0x00,0},
        {0x62,0xf1,0x7e,0x08,0x11,0x00,0},
        {0x62,0xf1,0x6e,0x08,0x10,0xcb,0}
    };
    static const uint8_t modern_sizes[] = {4,4,4,4,6,6,6};
    static const cdisasm_x86_form_id forms[] = {
        5941,5939,5942,5943,5944,5940,5945
    };
    size_t index;
...
#if USE_EXTRA_OPCODES
    {
        static const uint8_t legacy[][4] = {
            {0xf3,0x0f,0x10,0x01}, {0xf3,0x0f,0x10,0xc1},
            {0xf3,0x0f,0x11,0x01}, {0xf3,0x0f,0x11,0xc1}
        };
        cdisasm_x86_decode_flags flags =
            one_bit(CDISASM_X86_DECODE_BIT_SSE);
        ...
        for (index = 0u; index < 4u; ++index) {
            ...
            EXPECT(instruction.name_id == CDISASM_X86_NAME_MOVSS);
            EXPECT((instruction.opcode_flags
...
static void test_complete_vex_allocated_domain(void)
{
...
#endif
                                ++per_form[form_id - UINT16_C(5939)];
                                ++allocated;
                            }
                        }
                    }
    ...
    EXPECT(per_form[5939u - 5939u] == UINT32_C(10752));
    EXPECT(per_form[5941u - 5939u] == UINT32_C(10752));
    EXPECT(per_form[5942u - 5939u] == UINT32_C(55296));
    EXPECT(per_form[5943u - 5939u] == UINT32_C(55296));
}
...
static void test_complete_evex_allocated_domain(void)
{
    ...
    EXPECT(per_form[5940u - 5939u] == UINT32_C(331776));
    EXPECT(per_form[5944u - 5939u] == UINT32_C(622080));
    EXPECT(per_form[5945u - 5939u] == UINT32_C(6635520));
}

static void test_factored_reserved_and_collisions(void)
{
    uint32_t reserved = 0u;
    uint32_t collisions = 4u; /* Four legacy MOVSS direction/mod forms. */
#if USE_EXTRA_OPCODES
    cdisasm_x86_decode_flags flags = all_flags(CDISASM_MODE_64);
#endif
    ...
    /* EVEX P1 owns only F3/W0; U=0 is address-only and memory vvvv is
     * fixed.  Other pp values are the UPS/UPD/SD collision selectors. */
    {
        unsigned int value;
...
{
    static const uint8_t vex[] = {0xc5,0xea,0x10,0xcb};
    static const uint8_t evex[] = {0x62,0xf1,0x6e,0x08,0x10,0xcb};
    static const uint8_t b4_memory[] = {0x62,0xf9,0x7e,0x08,0x10,0x00};
#if USE_EXTRA_OPCODES
    static const uint8_t b4_register[] = {0x62,0xf9,0x6e,0x08,0x10,0xcb};
    static const uint8_t x4_memory[] = {
        0x62,0xf1,0x7a,0x08,0x10,0x04,0xa4
    };
    static const uint8_t addr32_b4[] = {
        0x67,0x62,0xf9,0x7e,0x08,0x10,0x00
    };
    static const uint8_t disp8_load[] = {
        0x62,0xf1,0x7e,0x89,0x10,0x40,0xff
    };
    static const uint8_t disp8_store[] = {
        0x62,0xf1,0x7e,0x09,0x11,0x40,0xff
    };
    static const uint8_t non64_vex[] = {0xc4,0xc1,0x3a,0x10,0xcb};
    static const uint8_t non64_evex[] = {0x62,0xc1,0x2e,0x08,0x10,0xcb};
    ...
    check_modern(&instruction, decoded_size, UINT16_C(5942), 0, 0, 0u, 0);
    ...
    check_modern(&instruction, decoded_size, UINT16_C(5945), 1, 0, 0u, 0);
    ...
    check_modern(&instruction, decoded_size, UINT16_C(5944), 1, 1, 0u, 0);
    EXPECT(instruction.opcode[1].base_reg == CDISASM_X86_REG_R16);
    instruction = decode(CDISASM_CPU_X86, CDISASM_MODE_64,
        b4_register, sizeof(b4_register), &scalar_apx, &decoded_size);
    check_modern(&instruction, decoded_size, UINT16_C(5945), 1, 1, 0u, 0);
    ...
    check_modern(&instruction, decoded_size, UINT16_C(5944), 1, 0, 1u, 1);
    EXPECT(instruction.opcode[1].imm == (uint64_t)INT64_C(-4));
    instruction = decode(CDISASM_CPU_X86, CDISASM_MODE_64,
        disp8_store, sizeof(disp8_store), &scalar, &decoded_size);
    check_modern(&instruction, decoded_size, UINT16_C(5940), 1, 0, 1u, 0);
    EXPECT(instruction.opcode[0].imm == (uint64_t)INT64_C(-4));
...
static void test_invalid_and_truncated(void)
{
    static const uint8_t invalid[][7] = {
        {0xc5,0xf2,0x10,0x00,0,0,0},       /* memory vvvv */
        {0x62,0xf1,0xfe,0x08,0x10,0x00,0}, /* W1 */
        {0x62,0xf1,0x7e,0x18,0x10,0x00,0}, /* b */
        {0x62,0xf1,0x7e,0x68,0x10,0x00,0}, /* LL3 */
        {0x62,0xf1,0x7e,0x88,0x10,0x00,0}, /* z with k0 */
        {0x62,0xf1,0x7e,0x89,0x11,0x00,0}, /* store zeroing */
        {0x62,0xf1,0x7e,0x00,0x10,0x00,0}, /* memory V' */
        {0x62,0xf1,0x6e,0x00,0x10,0xcb,0}, /* non-long V' below */
        {0x62,0xf1,0x6a,0x08,0x10,0xcb,0}  /* U0 register */
    };
    static const size_t sizes[] = {4,6,6,6,6,6,6,6,6};
    static const uint8_t missing_vex_modrm[] = {0xc5,0xf2,0x10};
    static const uint8_t missing_evex_modrm[] = {
        0x62,0xf1,0xfe,0x18,0x10
    };
    static const uint8_t bad_control_missing_sib[] = {
        0x62,0xf1,0xfe,0x18,0x10,0x04
    };
    size_t index;

    for (index = 0u; index < 7u; ++index) {
        expect_error("reserved VMOVSS selector", CDISASM_CPU_X86,
            CDISASM_MODE_64, invalid[index], sizes[index], NULL,
            CDISASM_STATUS_INVALID_INSTRUCTION);
    }
    expect_error("EVEX V' outside long", CDISASM_CPU_X86,
        CDISASM_MODE_32, invalid[7], sizes[7], NULL,
        CDISASM_STATUS_INVALID_INSTRUCTION);
    expect_error("EVEX U0 register", CDISASM_CPU_X86,
        CDISASM_MODE_64, invalid[8], sizes[8], NULL,
        CDISASM_STATUS_INVALID_INSTRUCTION);
...
static void test_formatting(void)
{
    ...
    static const uint8_t vex[] = {0xc5,0xea,0x11,0xcb};
    ...
    EXPECT(cdisasm_x86_format(&instruction, CDISASM_FORMAT_SYNTAX_INTEL,
        output, sizeof(output)) == strlen("vmovss xmm3, xmm2, xmm1"));
    EXPECT(strcmp(output, "vmovss xmm3, xmm2, xmm1") == 0);
    EXPECT(cdisasm_x86_format(&instruction, CDISASM_FORMAT_SYNTAX_ATT,
        output, sizeof(output)) == strlen("vmovss %xmm1, %xmm2, %xmm3"));
    EXPECT(strcmp(output, "vmovss %xmm1, %xmm2, %xmm3") == 0);

    instruction = decode(CDISASM_CPU_X86, CDISASM_MODE_64,
        load, sizeof(load), &scalar, &decoded_size);
    EXPECT(cdisasm_x86_format(&instruction, CDISASM_FORMAT_SYNTAX_INTEL,
        output, sizeof(output))
        == strlen("vmovss xmm0 {k1}{z}, dword ptr [rax - 0x4]"));
    EXPECT(strcmp(output,
        "vmovss xmm0 {k1}{z}, dword ptr [rax - 0x4]") == 0);
    instruction = decode(CDISASM_CPU_X86, CDISASM_MODE_64,
        store, sizeof(store), &scalar, &decoded_size);
    EXPECT(cdisasm_x86_format(&instruction, CDISASM_FORMAT_SYNTAX_INTEL,
        output, sizeof(output))
        == strlen("vmovss dword ptr [rax - 0x4] {k1}, xmm0"));
    EXPECT(strcmp(output,
        "vmovss dword ptr [rax - 0x4] {k1}, xmm0") == 0);
#endif
}
...
#undef RUN_TEST
    ...
    if (failures != 0) {
        fprintf(stderr, "%d VMOVSS test(s) failed\n", failures);
        return 1;
    }
    puts("x86 VMOVSS tests passed (7 forms; allocated=7,721,472; "
