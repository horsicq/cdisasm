...
static const form_desc forms[10] = {
    {0xc4408000u,CDISASM_ARM_NAME_LD1SB,3435u,1u},
    {0xc4c08000u,CDISASM_ARM_NAME_LD1SH,3436u,2u},
    {0xc5408000u,CDISASM_ARM_NAME_LD1SW,3437u,4u},
    {0xc440a000u,CDISASM_ARM_NAME_LDFF1SB,3442u,1u},
    {0xc4c0a000u,CDISASM_ARM_NAME_LDFF1SH,3443u,2u},
    {0xc540a000u,CDISASM_ARM_NAME_LDFF1SW,3444u,4u},
    {0xc5c0e000u,CDISASM_ARM_NAME_LDFF1D,3445u,8u},
    {0xc440e000u,CDISASM_ARM_NAME_LDFF1B,3446u,1u},
    {0xc4c0e000u,CDISASM_ARM_NAME_LDFF1H,3447u,2u},
    {0xc540e000u,CDISASM_ARM_NAME_LDFF1W,3448u,4u}
};
#endif
static int failures;
...
{
    unsigned f,zm,pg,rn,zt;
    for(f=0;f<10;f++)for(zm=0;zm<32;zm++)for(pg=0;pg<8;pg++)
    for(rn=0;rn<32;rn++)for(zt=0;zt<32;zt++){
        cdisasm_arm_instruction i;
#if USE_EXTRA_OPCODES
        uint32_t w=forms[f].value|(zm<<16)|(pg<<10)|(rn<<5)|zt;
        ...
        m.type=CDISASM_OPERAND_MEMORY;m.base_reg=rn==31u?CDISASM_ARM_REG_SP:CDISASM_ARM_REG_X0+rn;m.index_reg=CDISASM_ARM_REG_Z0+zm;m.size=forms[f].size;m.access=CDISASM_OPERAND_ACCESS_READ;
        E(memcmp(&i.operand[0],&d,sizeof(d))==0);E(memcmp(&i.operand[1],&p,sizeof(p))==0);E(memcmp(&i.operand[2],&m,sizeof(m))==0);
#else
        uint32_t w=UINT32_C(0xc4408000)|(zm<<16)|(pg<<10)|(rn<<5)|zt;
...
{
    cdisasm_arm_instruction i;E(dw(UINT32_C(0xc55de79b),CDISASM_ARM_CPU_CORTEX_A53,&i)==0u);
#if USE_EXTRA_OPCODES
    E(eo(&i,CDISASM_STATUS_INVALID_INSTRUCTION));E(dw(UINT32_C(0xc55de79b),CDISASM_ARM_CPU_FUJITSU_A64FX,&i)==4u);
#if USE_DISASM_FORMAT
    {char t[96];E(cdisasm_arm_format(&i,0,t,sizeof(t))==strlen("ldff1w {z27.d}, p1/z, [x28, z29.d]"));E(strcmp(t,"ldff1w {z27.d}, p1/z, [x28, z29.d]")==0);i.operand[2].scale=1u;E(cdisasm_arm_format(&i,0,t,sizeof(t))==0u);}
#endif
#else
    E(eo(&i,CDISASM_STATUS_UNSUPPORTED_INSTRUCTION));
#endif
}
int main(void){domains();gates_format();if(failures)return fprintf(stderr,"%d failures\n",failures),1;puts("SVE 64-bit x64-unscaled gather-load tests passed (10 exact leaves)");return 0;}
