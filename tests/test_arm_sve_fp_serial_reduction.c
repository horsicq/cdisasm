...
_Static_assert(CDISASM_ARM_NAME_FADDA == UINT16_C(420),
               "SVE FADDA ID must be append-only");
_Static_assert(CDISASM_ARM_NAME_FADDA < CDISASM_ARM_NAME_COUNT,
               "SVE FADDA ID is outside the catalog");
_Static_assert(CDISASM_ARM_NAME_COUNT >= UINT16_C(421),
               "SVE FADDA name-count floor changed");
...
static uint32_t fadda_word(unsigned size_code, unsigned zm,
                           unsigned pg, unsigned vd)
{
    return UINT32_C(0x65182000)
    ...
    cdisasm_arm_instruction *instruction)
{
    ...
    return cdisasm_arm_decode(cpu_id, mode, bytes, code_size,
        UINT64_C(0x116000), options, instruction);
}
                          ...
                          cdisasm_operand_access access)
{
    ...
    expected.size = (uint8_t)(UINT32_C(1) << size_code);
...
static int metadata_matches(const cdisasm_arm_instruction *instruction,
                            ...
                            unsigned zm, unsigned pg, unsigned vd)
{
    ...
    return instruction->last_error_id == CDISASM_STATUS_OK
        ...
        && instruction->address == UINT64_C(0x116000)
        ...
        && scalar_matches(&instruction->operand[2], vd, size_code,
...
static void test_exhaustive_exact_envelope(void)
{
    ...
    for (size_code = 0u; size_code < 4u; ++size_code) {
        ...
        for (pg = 0u; pg < 8u; ++pg) {
            ...
            for (zm = 0u; zm < 32u; ++zm) {
                ...
                for (vd = 0u; vd < 32u; ++vd) {
                    cdisasm_arm_instruction instruction;
                    uint32_t word = fadda_word(size_code, zm, pg, vd);
                    uint32_t decoded;
    ...
    EXPECT(allocated_words == UINT32_C(24576));
    EXPECT(reserved_words == UINT32_C(8192));
    EXPECT(allocated_words + reserved_words == UINT32_C(32768));
}
...
{
    uint32_t word = fadda_word(3u, 13u, 3u, 12u);
    uint32_t reserved = fadda_word(0u, 13u, 3u, 12u);
    ...
    EXPECT(cdisasm_arm_decode(CDISASM_ARM_CPU_ANY, CDISASM_ARM_MODE_A64,
        bytes, sizeof(bytes), UINT64_C(0x116000),
...
{
    expect_format(fadda_word(1u, 13u, 3u, 12u),
        "fadda h12, p3, h12, z13.h");
    expect_format(fadda_word(2u, 13u, 3u, 12u),
        "fadda s12, p3, s12, z13.s");
    expect_format(fadda_word(3u, 13u, 3u, 12u),
        "fadda d12, p3, d12, z13.d");

    {
        ...
        EXPECT(strcmp(text, "FADDA d12, p3, d12, z13.d") == 0);
    }
}
#endif
...
int main(void)
{
    ...
    if (failures != 0) {
        fprintf(stderr,
            "%d ARM SVE FADDA serial-reduction test(s) failed\n",
            failures);
        return 1;
    }
    printf("ARM SVE FADDA serial-reduction tests passed "
           "(USE_EXTRA_OPCODES=%d, owned=32768, allocated=24576, "
           "reserved=8192)\n", USE_EXTRA_OPCODES);
    return 0;
}
