...
static uint32_t ftsmul_word(unsigned size, unsigned zm, unsigned zn,
    unsigned zd)
{
    return UINT32_C(0x65000c00) | (size << 22) | (zm << 16)
        | (zn << 5) | zd;
}
    ...
    cdisasm_arm_instruction *instruction)
{
    ...
    return cdisasm_arm_decode(cpu, CDISASM_ARM_MODE_A64, bytes, 4u,
        UINT64_C(0x65000c00), CDISASM_ARM_DECODE_OPTION_NONE, instruction);
}
...
static void check(unsigned size, unsigned zm, unsigned zn, unsigned zd)
{
    cdisasm_arm_instruction instruction;
    uint32_t word = ftsmul_word(size, zm, zn, zd);
    ...
    EXPECT(instruction.name_id == CDISASM_ARM_NAME_FTSMUL);
    EXPECT(instruction.form_id == (cdisasm_arm_form_id)3060u);
...
{
    uint32_t word = ftsmul_word(2u, 14u, 13u, 12u);
    cdisasm_arm_instruction instruction;
    memset(&instruction, 0xa5, sizeof(instruction));
#if USE_EXTRA_OPCODES
...
#  if USE_DISASM_FORMAT
    {
        ...
        EXPECT(cdisasm_arm_format(&instruction, 0u, text, sizeof(text))
            == strlen("ftsmul z12.s, z13.s, z14.s"));
        EXPECT(strcmp(text, "ftsmul z12.s, z13.s, z14.s") == 0);
    }
#  endif
#else
    EXPECT(decode_word(word, CDISASM_ARM_CPU_ANY, &instruction) == 0u);
