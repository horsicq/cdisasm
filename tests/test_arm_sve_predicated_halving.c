...
_Static_assert(CDISASM_ARM_NAME_SHADD == UINT16_C(1378),
               "predicated halving SHADD ID moved");
_Static_assert(CDISASM_ARM_NAME_SHSUB == UINT16_C(1388),
               "predicated halving SHSUB ID moved");
_Static_assert(CDISASM_ARM_NAME_SRHADD == UINT16_C(1520),
               "predicated halving SRHADD ID moved");
_Static_assert(CDISASM_ARM_NAME_SHSUBR == UINT16_C(1391),
               "predicated halving SHSUBR ID moved");
_Static_assert(CDISASM_ARM_NAME_UHADD == UINT16_C(1754),
               "predicated halving UHADD ID moved");
_Static_assert(CDISASM_ARM_NAME_UHSUB == UINT16_C(1759),
               "predicated halving UHSUB ID moved");
_Static_assert(CDISASM_ARM_NAME_URHADD == UINT16_C(1824),
               "predicated halving URHADD ID moved");
_Static_assert(CDISASM_ARM_NAME_UHSUBR == UINT16_C(1762),
               "predicated halving UHSUBR ID moved");
_Static_assert(CDISASM_ARM_FORM_AARCHMRS_2026_03_LAST >= UINT16_C(2686),
               "AARCHMRS form inventory no longer covers this family");

#if USE_EXTRA_OPCODES
typedef struct halving_operation {
    cdisasm_arm_name_id name_id;
    cdisasm_arm_form_id form_id;
    const char *mnemonic;
} halving_operation;

/* Indexed by the exact U:R:S control in the pinned AARCHMRS 2026-03 path
 * A64/sve/sve_intx_predicated/sve_intx_pred_arith_binary. */
static const halving_operation operations[8] = {
    { CDISASM_ARM_NAME_SHADD,  UINT16_C(2679), "shadd" },
    { CDISASM_ARM_NAME_UHADD,  UINT16_C(2683), "uhadd" },
    { CDISASM_ARM_NAME_SHSUB,  UINT16_C(2680), "shsub" },
    { CDISASM_ARM_NAME_UHSUB,  UINT16_C(2684), "uhsub" },
    { CDISASM_ARM_NAME_SRHADD, UINT16_C(2681), "srhadd" },
    { CDISASM_ARM_NAME_URHADD, UINT16_C(2685), "urhadd" },
    { CDISASM_ARM_NAME_SHSUBR, UINT16_C(2682), "shsubr" },
    { CDISASM_ARM_NAME_UHSUBR, UINT16_C(2686), "uhsubr" }
};
#endif
    ...
    cdisasm_arm_instruction *instruction)
{
    ...
    return cdisasm_arm_decode(
        cpu_id, mode, bytes, code_size, UINT64_C(0x11d000),
        options, instruction);
}
...
static void expect_success_metadata(
    const cdisasm_arm_instruction *instruction,
    const halving_operation *operation,
    uint32_t word,
    unsigned size_code,
    unsigned pg,
    unsigned zm,
    unsigned zdn)
{
    ...
    EXPECT(instruction->address == UINT64_C(0x11d000));
...
static void test_feature_and_profile_routes(void)
{
    ...
    for (operation = 0u; operation < 8u; ++operation) {
        uint32_t word = halving_word(operation, 2u, 3u, 13u, 7u);
...
{
    uint32_t word = halving_word(7u, 3u, 7u, 31u, 31u);
    ...
    /* Exact preceding form 2678 and following form 2687 are independent;
     * neither neighbor can be swallowed by the halving parent. */
    memset(&other, 0xa5, sizeof(other));
#if USE_EXTRA_OPCODES
    EXPECT(decode_word(
        UINT32_C(0x4445a020), CDISASM_ARM_CPU_ANY,
        CDISASM_ARM_MODE_A64, 4u, CDISASM_ARM_DECODE_OPTION_NONE,
        &other) == 4u);
    EXPECT(other.form_id == UINT16_C(2678));
#else
    EXPECT(decode_word(
        UINT32_C(0x4445a020), CDISASM_ARM_CPU_ANY,
        CDISASM_ARM_MODE_A64, 4u, CDISASM_ARM_DECODE_OPTION_NONE,
...
#if USE_EXTRA_OPCODES
    EXPECT(decode_word(
        UINT32_C(0x4410a020), CDISASM_ARM_CPU_ANY,
        CDISASM_ARM_MODE_A64, 4u, CDISASM_ARM_DECODE_OPTION_NONE,
        &other) == 4u);
    EXPECT(other.name_id == CDISASM_ARM_NAME_SUBP);
    EXPECT(other.form_id == UINT16_C(2687));
#else
    EXPECT(decode_word(
        UINT32_C(0x4410a020), CDISASM_ARM_CPU_ANY,
        CDISASM_ARM_MODE_A64, 4u, CDISASM_ARM_DECODE_OPTION_NONE,
    ...
    decoded = cdisasm_arm_decode(
        ...
        bytes, sizeof(bytes), UINT64_C(0x11d000),
...
static void test_formatter_controls(void)
{
...

    {
        ...
        static const char expected[] =
            "UHSUBR z31.d, p7/m, z31.d, z31.d";

        memset(&instruction, 0xa5, sizeof(instruction));
        EXPECT(decode_word(
            halving_word(7u, 3u, 7u, 31u, 31u),
        ...
        forged = instruction;
        forged.form_id = UINT16_C(2685);
        EXPECT(cdisasm_arm_format(
            &forged, CDISASM_FORMAT_SYNTAX_0,
            text, sizeof(text)) == 0u);
        forged = instruction;
        forged.name_id = CDISASM_ARM_NAME_URHADD;
        EXPECT(cdisasm_arm_format(
            &forged, CDISASM_FORMAT_SYNTAX_0,
            text, sizeof(text)) == 0u);
        forged = instruction;
        forged.raw_instruction = halving_word(
            6u, 3u, 7u, 31u, 31u);
        EXPECT(cdisasm_arm_format(
            &forged, CDISASM_FORMAT_SYNTAX_0,
            text, sizeof(text)) == 0u);
...
int main(void)
{
    ...
    if (failures != 0) {
        fprintf(stderr,
                "%d ARM SVE predicated halving-arithmetic test(s) failed\n",
                failures);
        return 1;
    }
    printf("ARM SVE predicated halving-arithmetic tests passed "
