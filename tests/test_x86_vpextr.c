#include "cdisasm/cdisasm_x86.h"
#if USE_DISASM_FORMAT
#  include "cdisasm/cdisasm_format.h"
#endif

#include <stdint.h>
#include <stdio.h>
#include <string.h>

static int failures;

#define EXPECT(expression) do { if (!(expression)) { \
    fprintf(stderr, "%s:%d: %s\n", __FILE__, __LINE__, #expression); \
    ++failures; } } while (0)

_Static_assert(CDISASM_X86_NAME_VPEXTRB == UINT16_C(1843)
        && CDISASM_X86_NAME_VPEXTRD == UINT16_C(1844)
        && CDISASM_X86_NAME_VPEXTRQ == UINT16_C(1845)
        && CDISASM_X86_NAME_VPEXTRW == UINT16_C(1846)
        && CDISASM_X86_NAME_VPEXTRW_C5 == UINT16_C(1847),
    "VPEXTR name IDs changed");
_Static_assert(CDISASM_X86_GROUP_AVX == UINT16_C(37)
        && CDISASM_X86_DECODE_BIT_AVX == UINT32_C(8),
    "VPEXTR AVX IDs changed");

static int is_error_only(const cdisasm_instruction *instruction,
    cdisasm_status status)
{
    cdisasm_instruction expected;

    memset(&expected, 0, sizeof(expected));
    expected.last_error_id = (uint8_t)status;
    return memcmp(instruction, &expected, sizeof(expected)) == 0;
}

static cdisasm_instruction decode(cdisasm_cpu_id cpu, cdisasm_mode mode,
    const uint8_t *code, size_t size,
    const cdisasm_x86_decode_flags *flags, uint32_t *decoded_size)
{
    cdisasm_instruction instruction;

    memset(&instruction, 0xa5, sizeof(instruction));
    *decoded_size = cdisasm_x86_decode(cpu, mode, code, size,
        UINT64_C(0x1000), flags, &instruction);
    return instruction;
}

static void expect_error_cpu(const char *label, cdisasm_cpu_id cpu,
    cdisasm_mode mode, const uint8_t *code, size_t size,
    const cdisasm_x86_decode_flags *flags, cdisasm_status status)
{
    uint32_t decoded_size;
    cdisasm_instruction instruction = decode(cpu, mode, code, size,
        flags, &decoded_size);

    if (decoded_size != 0u || !is_error_only(&instruction, status)) {
        fprintf(stderr, "%s: got size/status %u/%u, expected 0/%u\n",
            label, (unsigned int)decoded_size,
            (unsigned int)instruction.last_error_id, (unsigned int)status);
    }
    EXPECT(decoded_size == 0u);
    EXPECT(is_error_only(&instruction, status));
}

static void expect_error(const char *label, cdisasm_mode mode,
    const uint8_t *code, size_t size,
    const cdisasm_x86_decode_flags *flags, cdisasm_status status)
{
    expect_error_cpu(label, CDISASM_CPU_X86, mode, code, size,
        flags, status);
}

#if USE_EXTRA_OPCODES
static cdisasm_x86_decode_flags all_flags(cdisasm_mode mode)
{
    cdisasm_x86_decode_flags flags;

    EXPECT(cdisasm_x86_cpu_decode_flag_mask(
        CDISASM_CPU_X86, mode, &flags) == CDISASM_STATUS_OK);
    return flags;
}

static cdisasm_x86_decode_flags selected_flags(int avx, int avx2)
{
    cdisasm_x86_decode_flags flags =
        CDISASM_X86_DECODE_FLAGS_NONE_INITIALIZER;

    if (avx) {
        EXPECT(cdisasm_decode_flags_set_bit(
            &flags, CDISASM_X86_DECODE_BIT_AVX));
    }
    if (avx2) {
        EXPECT(cdisasm_decode_flags_set_bit(
            &flags, CDISASM_X86_DECODE_BIT_AVX2));
    }
    return flags;
}

static cdisasm_x86_form_id expected_form(cdisasm_x86_name_id name,
    int register_form, int map1_c5)
{
    if (map1_c5) {
        return UINT16_C(6979);
    }
    switch (name) {
        case CDISASM_X86_NAME_VPEXTRB:
            return register_form ? UINT16_C(6966) : UINT16_C(6968);
        case CDISASM_X86_NAME_VPEXTRD:
            return register_form ? UINT16_C(6970) : UINT16_C(6972);
        case CDISASM_X86_NAME_VPEXTRQ:
            return register_form ? UINT16_C(6974) : UINT16_C(6976);
        case CDISASM_X86_NAME_VPEXTRW:
            return register_form ? UINT16_C(6978) : UINT16_C(6983);
        default:
            EXPECT(0);
            return CDISASM_X86_FORM_NONE;
    }
}

static unsigned int memory_size(cdisasm_x86_name_id name)
{
    switch (name) {
        case CDISASM_X86_NAME_VPEXTRB: return 1u;
        case CDISASM_X86_NAME_VPEXTRW: return 2u;
        case CDISASM_X86_NAME_VPEXTRD: return 4u;
        case CDISASM_X86_NAME_VPEXTRQ: return 8u;
        default:
            EXPECT(0);
            return 0u;
    }
}

static void check_instruction(const cdisasm_instruction *instruction,
    uint32_t decoded_size, cdisasm_mode mode, cdisasm_x86_name_id name,
    int register_form, int map1_c5)
{
    const unsigned int scalar_size = memory_size(name);
    const cdisasm_opcode *destination = &instruction->opcode[0];
    const cdisasm_opcode *source = &instruction->opcode[1];
    const cdisasm_opcode *immediate = &instruction->opcode[2];

    EXPECT(decoded_size >= 5u);
    EXPECT(instruction->last_error_id == CDISASM_STATUS_OK);
    EXPECT(instruction->name_id == name);
    EXPECT(instruction->form_id == expected_form(
        name, register_form, map1_c5));
    EXPECT(instruction->operand_count == 3u);
    EXPECT((instruction->opcode_flags
        & (CDISASM_X86_INSTRUCTION_FLAG_GENERATED_FALLBACK
            | CDISASM_X86_INSTRUCTION_FLAG_OPERANDS_OPAQUE
            | CDISASM_X86_INSTRUCTION_FLAG_READS_STATUS_FLAGS
            | CDISASM_X86_INSTRUCTION_FLAG_WRITES_STATUS_FLAGS)) == 0u);
    EXPECT((instruction->opcode_flags
        & ~(CDISASM_PREFIX_VEX | CDISASM_PREFIX_ADDRESS_SIZE
            | CDISASM_PREFIX_SEGMENT)) == 0u);
    EXPECT((instruction->opcode_flags & CDISASM_PREFIX_VEX) != 0u);
    EXPECT(instruction->encoding.prefix_size >= (map1_c5 ? 2u : 3u));
    EXPECT(instruction->encoding.opcode_size == 1u);
    EXPECT(instruction->encoding.modrm_offset
        == instruction->encoding.opcode_offset + 1u);
    EXPECT(instruction->encoding.immediate_count == 1u);
    EXPECT(instruction->encoding.immediate_size[0] == 1u);
    EXPECT(instruction->encoding.immediate_offset[0]
        == instruction->opcode_size - 1u);
    EXPECT(instruction->encoding.selector_offset == 0u);

    EXPECT(destination->type == (register_form
        ? CDISASM_OPERAND_REGISTER : CDISASM_OPERAND_MEMORY));
    EXPECT(destination->size == (register_form
        ? (name == CDISASM_X86_NAME_VPEXTRQ ? 8u : 4u)
        : scalar_size));
    EXPECT(destination->access == CDISASM_OPERAND_ACCESS_WRITE);
    EXPECT(destination->broadcast == CDISASM_X86_BROADCAST_NONE);
    EXPECT((destination->flags
        & (CDISASM_OPERAND_FLAG_IMPLICIT
            | CDISASM_OPERAND_FLAG_ADDRESS_ONLY)) == 0u);
    if (register_form && mode != CDISASM_MODE_64) {
        EXPECT(destination->reg <= CDISASM_X86_REG_EDI);
    }
    EXPECT(source->type == CDISASM_OPERAND_REGISTER);
    EXPECT(source->reg >= CDISASM_X86_REG_XMM0);
    EXPECT(source->reg <= CDISASM_X86_REG_XMM15);
    EXPECT(source->size == 16u);
    EXPECT(source->access == CDISASM_OPERAND_ACCESS_READ);
    EXPECT(source->flags == 0u);
    EXPECT(source->broadcast == CDISASM_X86_BROADCAST_NONE);
    if (mode != CDISASM_MODE_64) {
        EXPECT(source->reg <= CDISASM_X86_REG_XMM7);
    }
    EXPECT(immediate->type == CDISASM_OPERAND_IMMEDIATE);
    EXPECT(immediate->size == 1u);
    EXPECT(immediate->access == CDISASM_OPERAND_ACCESS_READ);
    EXPECT(immediate->flags == 0u);
    EXPECT(immediate->broadcast == CDISASM_X86_BROADCAST_NONE);
    EXPECT(immediate->imm <= UINT8_MAX);
    EXPECT(instruction->mask_reg == CDISASM_X86_REG_NONE);
    EXPECT(instruction->mask_mode == CDISASM_X86_MASK_NONE);
    EXPECT(instruction->rounding == CDISASM_X86_ROUNDING_NONE);
    EXPECT(instruction->sae == CDISASM_X86_SAE_NONE);
    EXPECT(instruction->opcode_groups == CDISASM_GROUP_NONE);
    EXPECT(instruction->branch_target == 0u);
    EXPECT(cdisasm_instruction_has_x86_group(
        instruction, CDISASM_X86_GROUP_AVX));
    EXPECT(!cdisasm_instruction_has_x86_group(
        instruction, CDISASM_X86_GROUP_AVX2));
}
#endif

static void test_complete_c4_partition(void)
{
    static const cdisasm_mode modes[3] = {
        CDISASM_MODE_16, CDISASM_MODE_32, CDISASM_MODE_64};
    static const struct owned_row {
        uint8_t map;
        uint8_t opcode;
    } rows[] = {{3,0x14},{3,0x15},{3,0x16},{1,0xc5}};
    static const uint64_t expected_forms[9] = {
        1536u,4608u,1024u,3072u,512u,1536u,1536u,4608u,1536u};
    uint64_t form_counts[9] = {0};
    uint64_t allocated = 0u;
    uint64_t reserved = 0u;
    size_t row_index;
    size_t mode_index;

    for (row_index = 0u;
         row_index < sizeof(rows) / sizeof(rows[0]); ++row_index) {
        for (mode_index = 0u; mode_index < 3u; ++mode_index) {
            const int long_mode = modes[mode_index] == CDISASM_MODE_64;
            const unsigned int p0_count = long_mode ? 8u : 2u;
            unsigned int p0_index;
#if USE_EXTRA_OPCODES
            cdisasm_x86_decode_flags flags = all_flags(modes[mode_index]);
#endif

            for (p0_index = 0u; p0_index < p0_count; ++p0_index) {
                const uint8_t p0 = long_mode
                    ? (uint8_t)((p0_index << 5) | rows[row_index].map)
                    : (uint8_t)(UINT8_C(0xc0)
                        | (p0_index << 5) | rows[row_index].map);
                unsigned int p1;

                for (p1 = 0u; p1 <= UINT8_MAX; ++p1) {
                    const int selector_valid =
                        ((uint8_t)p1 & UINT8_C(0x7f)) == UINT8_C(0x79);
                    unsigned int modrm;

                    for (modrm = 0u; modrm <= UINT8_MAX; ++modrm) {
                        const int register_form =
                            (modrm & UINT8_C(0xc0)) == UINT8_C(0xc0);
                        const int map1_c5 = rows[row_index].map == 1u;
                        const int valid = selector_valid
                            && (!map1_c5 || register_form);
                        const uint8_t code[15] = {
                            0xc4,p0,(uint8_t)p1,rows[row_index].opcode,
                            (uint8_t)modrm,0x24,0x10,0x20,0x30,0x40,
                            0x50,0x60,0x70,0x80,0x90};
                        uint32_t decoded_size;
                        cdisasm_instruction instruction = decode(
                            CDISASM_CPU_X86, modes[mode_index], code,
                            sizeof(code),
#if USE_EXTRA_OPCODES
                            &flags,
#else
                            NULL,
#endif
                            &decoded_size);

                        if (valid) {
#if USE_EXTRA_OPCODES
                            cdisasm_x86_name_id name;
                            unsigned int form_index;

                            if (map1_c5) {
                                name = CDISASM_X86_NAME_VPEXTRW;
                                form_index = 8u;
                            } else if (rows[row_index].opcode == 0x14u) {
                                name = CDISASM_X86_NAME_VPEXTRB;
                                form_index = register_form ? 0u : 1u;
                            } else if (rows[row_index].opcode == 0x15u) {
                                name = CDISASM_X86_NAME_VPEXTRW;
                                form_index = register_form ? 6u : 7u;
                            } else if (((uint8_t)p1 & UINT8_C(0x80)) != 0u
                                && long_mode) {
                                name = CDISASM_X86_NAME_VPEXTRQ;
                                form_index = register_form ? 4u : 5u;
                            } else {
                                name = CDISASM_X86_NAME_VPEXTRD;
                                form_index = register_form ? 2u : 3u;
                            }
                            check_instruction(&instruction, decoded_size,
                                modes[mode_index], name, register_form,
                                map1_c5);
                            ++form_counts[form_index];
#else
                            EXPECT(decoded_size == 0u);
                            EXPECT(is_error_only(&instruction,
                                CDISASM_STATUS_UNSUPPORTED_INSTRUCTION));
#endif
                            ++allocated;
                        } else {
                            EXPECT(decoded_size == 0u);
                            EXPECT(is_error_only(&instruction,
                                CDISASM_STATUS_INVALID_INSTRUCTION));
                            ++reserved;
                        }
                    }
                }
            }
        }
    }
    EXPECT(allocated == UINT64_C(19968));
    EXPECT(reserved == UINT64_C(3125760));
#if USE_EXTRA_OPCODES
    for (row_index = 0u;
         row_index < sizeof(expected_forms) / sizeof(expected_forms[0]);
         ++row_index) {
        EXPECT(form_counts[row_index] == expected_forms[row_index]);
    }
#else
    (void)expected_forms;
    (void)form_counts;
#endif
}

static void test_complete_c5_partition(void)
{
    static const cdisasm_mode modes[3] = {
        CDISASM_MODE_16, CDISASM_MODE_32, CDISASM_MODE_64};
    uint64_t allocated = 0u;
    uint64_t reserved = 0u;
    size_t mode_index;

    for (mode_index = 0u; mode_index < 3u; ++mode_index) {
        const int long_mode = modes[mode_index] == CDISASM_MODE_64;
        const unsigned int p1_count = long_mode ? 256u : 64u;
        unsigned int p1_index;
#if USE_EXTRA_OPCODES
        cdisasm_x86_decode_flags flags = all_flags(modes[mode_index]);
#endif

        for (p1_index = 0u; p1_index < p1_count; ++p1_index) {
            const uint8_t p1 = long_mode
                ? (uint8_t)p1_index
                : (uint8_t)(UINT8_C(0xc0) | p1_index);
            unsigned int modrm;

            for (modrm = 0u; modrm <= UINT8_MAX; ++modrm) {
                const int register_form =
                    (modrm & UINT8_C(0xc0)) == UINT8_C(0xc0);
                const int valid = (p1 == UINT8_C(0x79)
                        || p1 == UINT8_C(0xf9))
                    && register_form;
                const uint8_t code[15] = {
                    0xc5,p1,0xc5,(uint8_t)modrm,0x24,0x10,0x20,0x30,
                    0x40,0x50,0x60,0x70,0x80,0x90,0xa0};
                uint32_t decoded_size;
                cdisasm_instruction instruction = decode(CDISASM_CPU_X86,
                    modes[mode_index], code, sizeof(code),
#if USE_EXTRA_OPCODES
                    &flags,
#else
                    NULL,
#endif
                    &decoded_size);

                if (valid) {
#if USE_EXTRA_OPCODES
                    check_instruction(&instruction, decoded_size,
                        modes[mode_index], CDISASM_X86_NAME_VPEXTRW,
                        1, 1);
#else
                    EXPECT(decoded_size == 0u);
                    EXPECT(is_error_only(&instruction,
                        CDISASM_STATUS_UNSUPPORTED_INSTRUCTION));
#endif
                    ++allocated;
                } else {
                    EXPECT(decoded_size == 0u);
                    EXPECT(is_error_only(&instruction,
                        CDISASM_STATUS_INVALID_INSTRUCTION));
                    ++reserved;
                }
            }
        }
    }
    EXPECT(allocated == UINT64_C(256));
    EXPECT(reserved == UINT64_C(98048));
}

static void test_forms_gates_and_aliases(void)
{
    static const uint8_t b_reg[] = {0xc4,0xe3,0x79,0x14,0xca,0x05};
    static const uint8_t b_mem[] = {0xc4,0xe3,0xf9,0x14,0x00,0xa5};
    static const uint8_t d_reg[] = {0xc4,0xe3,0x79,0x16,0xca,0x05};
    static const uint8_t q_mem[] = {0xc4,0xe3,0xf9,0x16,0x00,0x01};
    static const uint8_t w3_reg[] = {0xc4,0xe3,0x79,0x15,0xca,0x05};
    static const uint8_t w3_mem[] = {0xc4,0xe3,0xf9,0x15,0x00,0xa5};
    static const uint8_t w1_c4[] = {0xc4,0xe1,0xf9,0xc5,0xd1,0x05};
    static const uint8_t w1_c5[] = {0xc5,0xf9,0xc5,0xd1,0x05};
    static const uint8_t high_map3[] = {0xc4,0x43,0x79,0x14,0xcb,0xa5};
    static const uint8_t high_map1[] = {0xc4,0x41,0x79,0xc5,0xd1,0x05};
    static const uint8_t nonlong_d_w1[] =
        {0xc4,0xc3,0xf9,0x16,0xca,0x05};
    static const uint8_t nonlong_w1_b_alias[] =
        {0xc4,0xc1,0xf9,0xc5,0xd1,0x05};
    static const uint8_t address[] =
        {0x67,0xc4,0xe3,0x79,0x14,0x00,0x05};
    static const uint8_t segment[] =
        {0x64,0xc4,0xe3,0x79,0x15,0x00,0x05};
#if !USE_EXTRA_OPCODES
    static const struct legal_case {
        const uint8_t *code;
        size_t size;
        cdisasm_mode mode;
    } cases[] = {
        {b_reg,sizeof(b_reg),CDISASM_MODE_64},
        {b_mem,sizeof(b_mem),CDISASM_MODE_64},
        {d_reg,sizeof(d_reg),CDISASM_MODE_64},
        {q_mem,sizeof(q_mem),CDISASM_MODE_64},
        {w3_reg,sizeof(w3_reg),CDISASM_MODE_64},
        {w3_mem,sizeof(w3_mem),CDISASM_MODE_64},
        {w1_c4,sizeof(w1_c4),CDISASM_MODE_64},
        {w1_c5,sizeof(w1_c5),CDISASM_MODE_64},
        {high_map3,sizeof(high_map3),CDISASM_MODE_64},
        {high_map1,sizeof(high_map1),CDISASM_MODE_64},
        {nonlong_d_w1,sizeof(nonlong_d_w1),CDISASM_MODE_32},
        {nonlong_w1_b_alias,sizeof(nonlong_w1_b_alias),CDISASM_MODE_32},
        {address,sizeof(address),CDISASM_MODE_64},
        {segment,sizeof(segment),CDISASM_MODE_64}};
    size_t index;
#endif

#if USE_EXTRA_OPCODES
    {
        static const struct positive_case {
            const uint8_t *code;
            size_t size;
            cdisasm_x86_name_id name;
            uint8_t register_form;
            uint8_t map1;
        } cases[] = {
            {b_reg,sizeof(b_reg),CDISASM_X86_NAME_VPEXTRB,1,0},
            {b_mem,sizeof(b_mem),CDISASM_X86_NAME_VPEXTRB,0,0},
            {d_reg,sizeof(d_reg),CDISASM_X86_NAME_VPEXTRD,1,0},
            {q_mem,sizeof(q_mem),CDISASM_X86_NAME_VPEXTRQ,0,0},
            {w3_reg,sizeof(w3_reg),CDISASM_X86_NAME_VPEXTRW,1,0},
            {w3_mem,sizeof(w3_mem),CDISASM_X86_NAME_VPEXTRW,0,0},
            {w1_c4,sizeof(w1_c4),CDISASM_X86_NAME_VPEXTRW,1,1},
            {w1_c5,sizeof(w1_c5),CDISASM_X86_NAME_VPEXTRW,1,1}};
        cdisasm_x86_decode_flags flags = all_flags(CDISASM_MODE_64);
        size_t index;

        for (index = 0u; index < sizeof(cases) / sizeof(cases[0]); ++index) {
            uint32_t decoded_size;
            cdisasm_instruction instruction = decode(CDISASM_CPU_X86,
                CDISASM_MODE_64, cases[index].code, cases[index].size,
                &flags, &decoded_size);

            check_instruction(&instruction, decoded_size, CDISASM_MODE_64,
                cases[index].name, cases[index].register_form,
                cases[index].map1);
        }
        {
            uint32_t decoded_size;
            cdisasm_instruction instruction = decode(CDISASM_CPU_X86,
                CDISASM_MODE_64, high_map3, sizeof(high_map3), &flags,
                &decoded_size);

            EXPECT(decoded_size == sizeof(high_map3));
            EXPECT(instruction.opcode[0].reg == CDISASM_X86_REG_R11D);
            EXPECT(instruction.opcode[1].reg == CDISASM_X86_REG_XMM9);
            instruction = decode(CDISASM_CPU_X86, CDISASM_MODE_64,
                high_map1, sizeof(high_map1), &flags, &decoded_size);
            EXPECT(decoded_size == sizeof(high_map1));
            EXPECT(instruction.opcode[0].reg == CDISASM_X86_REG_R10D);
            EXPECT(instruction.opcode[1].reg == CDISASM_X86_REG_XMM9);
            instruction = decode(CDISASM_CPU_X86, CDISASM_MODE_64,
                address, sizeof(address), &flags, &decoded_size);
            EXPECT(decoded_size == sizeof(address));
            EXPECT(instruction.opcode[0].base_reg == CDISASM_X86_REG_EAX);
            instruction = decode(CDISASM_CPU_X86, CDISASM_MODE_64,
                segment, sizeof(segment), &flags, &decoded_size);
            EXPECT(decoded_size == sizeof(segment));
            EXPECT(instruction.opcode[0].segment_reg == CDISASM_X86_REG_FS);
        }
    }
    {
        static const cdisasm_mode modes[2] = {
            CDISASM_MODE_16,CDISASM_MODE_32};
        size_t index;

        for (index = 0u; index < 2u; ++index) {
            cdisasm_x86_decode_flags flags = all_flags(modes[index]);
            uint32_t decoded_size;
            cdisasm_instruction instruction = decode(CDISASM_CPU_X86,
                modes[index], nonlong_d_w1, sizeof(nonlong_d_w1),
                &flags, &decoded_size);

            check_instruction(&instruction, decoded_size, modes[index],
                CDISASM_X86_NAME_VPEXTRD, 1, 0);
            EXPECT(instruction.opcode[0].reg == CDISASM_X86_REG_EDX);
            EXPECT(instruction.opcode[1].reg == CDISASM_X86_REG_XMM1);
            instruction = decode(CDISASM_CPU_X86, modes[index],
                nonlong_w1_b_alias, sizeof(nonlong_w1_b_alias),
                &flags, &decoded_size);
            check_instruction(&instruction, decoded_size, modes[index],
                CDISASM_X86_NAME_VPEXTRW, 1, 1);
            EXPECT(instruction.opcode[0].reg == CDISASM_X86_REG_EDX);
            EXPECT(instruction.opcode[1].reg == CDISASM_X86_REG_XMM1);
        }
    }
    {
        const cdisasm_x86_decode_flags none = selected_flags(0, 0);
        const cdisasm_x86_decode_flags avx = selected_flags(1, 0);
        const cdisasm_x86_decode_flags avx2 = selected_flags(0, 1);
        cdisasm_x86_decode_flags profile;
        uint32_t decoded_size;
        cdisasm_instruction instruction;

        expect_error("VPEXTR needs AVX", CDISASM_MODE_64,
            b_reg, sizeof(b_reg), &none,
            CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
        expect_error("AVX2 alone does not admit VPEXTR", CDISASM_MODE_64,
            b_reg, sizeof(b_reg), &avx2,
            CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
        instruction = decode(CDISASM_CPU_X86, CDISASM_MODE_64,
            b_reg, sizeof(b_reg), &avx, &decoded_size);
        EXPECT(decoded_size == sizeof(b_reg));
        EXPECT(instruction.form_id == UINT16_C(6966));
        EXPECT(cdisasm_x86_cpu_decode_flag_mask(CDISASM_CPU_SANDY_BRIDGE,
            CDISASM_MODE_64, &profile) == CDISASM_STATUS_OK);
        instruction = decode(CDISASM_CPU_SANDY_BRIDGE, CDISASM_MODE_64,
            b_reg, sizeof(b_reg), &profile, &decoded_size);
        EXPECT(decoded_size == sizeof(b_reg));
        expect_error_cpu("Westmere CPU rejects explicit AVX",
            CDISASM_CPU_WESTMERE, CDISASM_MODE_64,
            b_reg, sizeof(b_reg), &avx,
            CDISASM_STATUS_INVALID_INSTRUCTION);
    }
#else
    for (index = 0u; index < sizeof(cases) / sizeof(cases[0]); ++index) {
        expect_error("VPEXTR extras off", cases[index].mode,
            cases[index].code, cases[index].size, NULL,
            CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
    }
#endif
}

static void test_controls_and_precedence(void)
{
    static const uint8_t valid_c4[] = {0xc4,0xe3,0x79,0x14,0xca,0x05};
    static const uint8_t valid_c5[] = {0xc5,0xf9,0xc5,0xd1,0x05};
    size_t size;

    for (size = 1u; size < sizeof(valid_c4); ++size) {
        expect_error("truncated C4 VPEXTR", CDISASM_MODE_64,
            valid_c4, size, NULL, CDISASM_STATUS_TRUNCATED);
    }
    for (size = 1u; size < sizeof(valid_c5); ++size) {
        expect_error("truncated C5 VPEXTRW", CDISASM_MODE_64,
            valid_c5, size, NULL, CDISASM_STATUS_TRUNCATED);
    }
    expect_error("wrong pp", CDISASM_MODE_64,
        (const uint8_t[]){0xc4,0xe3,0x78,0x14,0xca,0x05}, 6u,
        NULL, CDISASM_STATUS_INVALID_INSTRUCTION);
    expect_error("L=1", CDISASM_MODE_64,
        (const uint8_t[]){0xc4,0xe3,0x7d,0x14,0xca,0x05}, 6u,
        NULL, CDISASM_STATUS_INVALID_INSTRUCTION);
    expect_error("NOVSR", CDISASM_MODE_64,
        (const uint8_t[]){0xc4,0xe3,0x71,0x14,0xca,0x05}, 6u,
        NULL, CDISASM_STATUS_INVALID_INSTRUCTION);
    expect_error("non-long NOVSR remains strict", CDISASM_MODE_32,
        (const uint8_t[]){0xc4,0xe3,0x39,0x14,0xca,0x05}, 6u,
        NULL, CDISASM_STATUS_INVALID_INSTRUCTION);
    expect_error("C5 memory form", CDISASM_MODE_64,
        (const uint8_t[]){0xc5,0xf9,0xc5,0x00,0x05}, 5u,
        NULL, CDISASM_STATUS_INVALID_INSTRUCTION);
    expect_error("reserved pp missing immediate", CDISASM_MODE_64,
        (const uint8_t[]){0xc4,0xe3,0x78,0x14,0xca}, 5u,
        NULL, CDISASM_STATUS_TRUNCATED);
    expect_error("reserved L missing SIB", CDISASM_MODE_64,
        (const uint8_t[]){0xc4,0xe3,0x7d,0x15,0x04}, 5u,
        NULL, CDISASM_STATUS_TRUNCATED);
    expect_error("NOVSR missing disp32", CDISASM_MODE_64,
        (const uint8_t[]){0xc4,0xe3,0x71,0x16,0x05,0x11,0x22,0x33}, 8u,
        NULL, CDISASM_STATUS_TRUNCATED);
    expect_error("C5 memory missing SIB", CDISASM_MODE_64,
        (const uint8_t[]){0xc5,0xf9,0xc5,0x04}, 4u,
        NULL, CDISASM_STATUS_TRUNCATED);
    {
        static const uint8_t encountered[5] = {0x66,0xf2,0xf3,0xf0,0x48};
        size_t index;

        for (index = 0u; index < sizeof(encountered); ++index) {
            const uint8_t map3_missing[7] = {
                encountered[index],0xc4,0xe3,0x79,0x14,0xca,0x00};
            const uint8_t map3_complete[7] = {
                encountered[index],0xc4,0xe3,0x79,0x14,0xca,0x05};
            const uint8_t map1_missing[7] = {
                encountered[index],0xc4,0xe1,0x79,0xc5,0xd1,0x00};
            const uint8_t map1_complete[7] = {
                encountered[index],0xc4,0xe1,0x79,0xc5,0xd1,0x05};
            const uint8_t c5_missing[6] = {
                encountered[index],0xc5,0xf9,0xc5,0xd1,0x00};
            const uint8_t c5_complete[6] = {
                encountered[index],0xc5,0xf9,0xc5,0xd1,0x05};

            expect_error("map3 encountered prefix truncated",
                CDISASM_MODE_64, map3_missing, sizeof(map3_missing) - 1u,
                NULL, CDISASM_STATUS_TRUNCATED);
            expect_error("map3 encountered prefix complete",
                CDISASM_MODE_64, map3_complete, sizeof(map3_complete),
                NULL, CDISASM_STATUS_INVALID_INSTRUCTION);
            expect_error("map1 encountered prefix truncated",
                CDISASM_MODE_64, map1_missing, sizeof(map1_missing) - 1u,
                NULL, CDISASM_STATUS_TRUNCATED);
            expect_error("map1 encountered prefix complete",
                CDISASM_MODE_64, map1_complete, sizeof(map1_complete),
                NULL, CDISASM_STATUS_INVALID_INSTRUCTION);
            expect_error("C5 encountered prefix truncated",
                CDISASM_MODE_64, c5_missing, sizeof(c5_missing) - 1u,
                NULL, CDISASM_STATUS_TRUNCATED);
            expect_error("C5 encountered prefix complete",
                CDISASM_MODE_64, c5_complete, sizeof(c5_complete),
                NULL, CDISASM_STATUS_INVALID_INSTRUCTION);
        }
    }
}

static void test_legacy_and_evex_isolation(void)
{
#if USE_EXTRA_OPCODES
    static const struct sibling {
        uint8_t code[7];
        uint8_t size;
        cdisasm_x86_name_id name;
        cdisasm_x86_form_id form;
        uint8_t evex;
    } siblings[] = {
        {{0x66,0x0f,0x3a,0x14,0xca,0x05,0},6,
            CDISASM_X86_NAME_PEXTRB,0,0},
        {{0x66,0x0f,0x3a,0x15,0xca,0x05,0},6,
            CDISASM_X86_NAME_PEXTRW,0,0},
        {{0x66,0x0f,0x3a,0x16,0xca,0x05,0},6,
            CDISASM_X86_NAME_PEXTRD,0,0},
        {{0x66,0x48,0x0f,0x3a,0x16,0xca,0x01},7,
            CDISASM_X86_NAME_PEXTRQ,0,0},
        {{0x66,0x0f,0xc5,0xd1,0x05,0,0},5,
            CDISASM_X86_NAME_PEXTRW,0,0},
        {{0x62,0xf3,0x7d,0x08,0x14,0xca,0x05},7,
            CDISASM_X86_NAME_VPEXTRB,UINT16_C(6967),1},
        {{0x62,0xf3,0x7d,0x08,0x14,0x00,0x05},7,
            CDISASM_X86_NAME_VPEXTRB,UINT16_C(6969),1},
        {{0x62,0xf3,0x7d,0x08,0x16,0xca,0x05},7,
            CDISASM_X86_NAME_VPEXTRD,UINT16_C(6971),1},
        {{0x62,0xf3,0x7d,0x08,0x16,0x00,0x05},7,
            CDISASM_X86_NAME_VPEXTRD,UINT16_C(6973),1},
        {{0x62,0xf3,0xfd,0x08,0x16,0xca,0x01},7,
            CDISASM_X86_NAME_VPEXTRQ,UINT16_C(6975),1},
        {{0x62,0xf3,0xfd,0x08,0x16,0x00,0x01},7,
            CDISASM_X86_NAME_VPEXTRQ,UINT16_C(6977),1},
        {{0x62,0xf3,0x7d,0x08,0x15,0xca,0x05},7,
            CDISASM_X86_NAME_VPEXTRW,UINT16_C(6980),1},
        {{0x62,0xf3,0x7d,0x08,0x15,0x00,0x05},7,
            CDISASM_X86_NAME_VPEXTRW,UINT16_C(6982),1},
        {{0x62,0xf1,0x7d,0x08,0xc5,0xd1,0x05},7,
            CDISASM_X86_NAME_VPEXTRW_C5,UINT16_C(6981),1}};
    cdisasm_x86_decode_flags flags = all_flags(CDISASM_MODE_64);
    size_t index;

    for (index = 0u; index < sizeof(siblings) / sizeof(siblings[0]);
         ++index) {
        uint32_t decoded_size;
        cdisasm_instruction instruction = decode(CDISASM_CPU_X86,
            CDISASM_MODE_64, siblings[index].code, siblings[index].size,
            &flags, &decoded_size);

        EXPECT(decoded_size == siblings[index].size);
        EXPECT(instruction.name_id == siblings[index].name);
        if (siblings[index].evex) {
            EXPECT(instruction.form_id == siblings[index].form);
            EXPECT((instruction.opcode_flags & CDISASM_PREFIX_EVEX) != 0u);
            EXPECT((instruction.opcode_flags & CDISASM_PREFIX_VEX) == 0u);
        } else {
            EXPECT(instruction.form_id < UINT16_C(6966)
                || instruction.form_id > UINT16_C(6983));
        }
    }
#endif
}

#if USE_EXTRA_OPCODES && USE_DISASM_FORMAT
static void expect_format_rejected(const cdisasm_instruction *instruction)
{
    char output[192] = {'x'};
    size_t intel_size;
    size_t att_size;

    intel_size = cdisasm_x86_format(instruction,
        CDISASM_FORMAT_SYNTAX_INTEL, output, sizeof(output));
    if (intel_size != 0u) {
        fprintf(stderr,
            "accepted forged VPEXTR name/form=%u/%u prefix=%u "
            "ops=%u types=%u,%u,%u sizes=%u,%u,%u regs=%u,%u "
            "access=%u,%u,%u groups=%u text=%s\n",
            (unsigned int)instruction->name_id,
            (unsigned int)instruction->form_id,
            (unsigned int)instruction->encoding.prefix_size,
            (unsigned int)instruction->operand_count,
            (unsigned int)instruction->opcode[0].type,
            (unsigned int)instruction->opcode[1].type,
            (unsigned int)instruction->opcode[2].type,
            (unsigned int)instruction->opcode[0].size,
            (unsigned int)instruction->opcode[1].size,
            (unsigned int)instruction->opcode[2].size,
            (unsigned int)instruction->opcode[0].reg,
            (unsigned int)instruction->opcode[1].reg,
            (unsigned int)instruction->opcode[0].access,
            (unsigned int)instruction->opcode[1].access,
            (unsigned int)instruction->opcode[2].access,
            (unsigned int)instruction->x86_group_count, output);
    }
    EXPECT(intel_size == 0u);
    EXPECT(output[0] == '\0');
    output[0] = 'x';
    att_size = cdisasm_x86_format(instruction, CDISASM_FORMAT_SYNTAX_ATT,
        output, sizeof(output));
    EXPECT(att_size == 0u);
    EXPECT(output[0] == '\0');
}

static void test_formatting_and_schema(void)
{
    static const struct format_case {
        uint8_t code[9];
        uint8_t size;
        const char *intel;
        const char *att;
    } cases[] = {
        {{0xc4,0xe3,0x79,0x14,0xca,0x05,0},6,
            "vpextrb edx, xmm1, 0x5", "vpextrb $0x5, %xmm1, %edx"},
        {{0xc4,0xe3,0x79,0x14,0x00,0x05,0},6,
            "vpextrb byte ptr [rax], xmm0, 0x5",
            "vpextrb $0x5, %xmm0, (%rax)"},
        {{0xc4,0xe3,0x79,0x16,0xca,0x05,0},6,
            "vpextrd edx, xmm1, 0x5", "vpextrd $0x5, %xmm1, %edx"},
        {{0xc4,0xe3,0xf9,0x16,0x00,0x01,0},6,
            "vpextrq qword ptr [rax], xmm0, 0x1",
            "vpextrq $0x1, %xmm0, (%rax)"},
        {{0xc4,0xe3,0x79,0x15,0xca,0x05,0},6,
            "vpextrw edx, xmm1, 0x5", "vpextrw $0x5, %xmm1, %edx"},
        {{0xc4,0xe3,0x79,0x15,0x00,0x05,0},6,
            "vpextrw word ptr [rax], xmm0, 0x5",
            "vpextrw $0x5, %xmm0, (%rax)"},
        {{0xc5,0xf9,0xc5,0xd1,0x05,0,0},5,
            "vpextrw edx, xmm1, 0x5", "vpextrw $0x5, %xmm1, %edx"},
        {{0xc4,0xe1,0x79,0xc5,0xd1,0x05,0},6,
            "vpextrw edx, xmm1, 0x5", "vpextrw $0x5, %xmm1, %edx"},
        {{0x62,0xf3,0x7d,0x08,0x14,0xca,0x05},7,
            "vpextrb edx, xmm1, 0x5", "vpextrb $0x5, %xmm1, %edx"},
        {{0x62,0xf3,0x7d,0x08,0x14,0x00,0x05},7,
            "vpextrb byte ptr [rax], xmm0, 0x5",
            "vpextrb $0x5, %xmm0, (%rax)"},
        {{0x62,0xf3,0x7d,0x08,0x16,0xca,0x05},7,
            "vpextrd edx, xmm1, 0x5", "vpextrd $0x5, %xmm1, %edx"},
        {{0x62,0xf3,0xfd,0x08,0x16,0x00,0x01},7,
            "vpextrq qword ptr [rax], xmm0, 0x1",
            "vpextrq $0x1, %xmm0, (%rax)"},
        {{0x62,0xf3,0x7d,0x08,0x15,0xca,0x05},7,
            "vpextrw edx, xmm1, 0x5", "vpextrw $0x5, %xmm1, %edx"},
        {{0x62,0xf3,0x7d,0x08,0x15,0x00,0x05},7,
            "vpextrw word ptr [rax], xmm0, 0x5",
            "vpextrw $0x5, %xmm0, (%rax)"},
        {{0x62,0xf1,0x7d,0x08,0xc5,0xd1,0x05},7,
            "vpextrw edx, xmm1, 0x5",
            "vpextrw $0x5, %xmm1, %edx"},
        {{0x64,0x67,0xc4,0xe3,0x79,0x14,0x00,0x05},8,
            "vpextrb byte ptr fs:[eax], xmm0, 0x5",
            "vpextrb $0x5, %xmm0, %fs:(%eax)"},
        {{0x64,0x67,0x62,0xf3,0x7d,0x08,0x14,0x00,0x05},9,
            "vpextrb byte ptr fs:[eax], xmm0, 0x5",
            "vpextrb $0x5, %xmm0, %fs:(%eax)"}};
    cdisasm_x86_decode_flags flags = all_flags(CDISASM_MODE_64);
    size_t index;

    for (index = 0u; index < sizeof(cases) / sizeof(cases[0]); ++index) {
        char output[192];
        uint32_t decoded_size;
        cdisasm_instruction instruction = decode(CDISASM_CPU_X86,
            CDISASM_MODE_64, cases[index].code, cases[index].size,
            &flags, &decoded_size);
        cdisasm_instruction forged;
        const int evex = (instruction.opcode_flags
            & CDISASM_PREFIX_EVEX) != 0u;
        const int map1 = instruction.form_id == UINT16_C(6979)
            || instruction.form_id == UINT16_C(6981);

        EXPECT(decoded_size == cases[index].size);
        EXPECT(cdisasm_x86_format(&instruction, CDISASM_FORMAT_SYNTAX_INTEL,
            output, sizeof(output)) == strlen(cases[index].intel));
        EXPECT(strcmp(output, cases[index].intel) == 0);
        EXPECT(cdisasm_x86_format(&instruction, CDISASM_FORMAT_SYNTAX_ATT,
            output, sizeof(output)) == strlen(cases[index].att));
        EXPECT(strcmp(output, cases[index].att) == 0);

        forged = instruction;
        forged.name_id = CDISASM_X86_NAME_VPHMINPOSUW;
        expect_format_rejected(&forged);
        forged = instruction;
        forged.form_id = CDISASM_X86_FORM_NONE;
        expect_format_rejected(&forged);
        forged = instruction;
        forged.operand_count = 2u;
        expect_format_rejected(&forged);
        forged = instruction;
        if (evex) {
            forged.opcode_flags &=
                ~CDISASM_X86_INSTRUCTION_FLAG_GENERATED_FALLBACK;
        } else {
            forged.opcode_flags |=
                CDISASM_X86_INSTRUCTION_FLAG_GENERATED_FALLBACK;
        }
        expect_format_rejected(&forged);
        forged = instruction;
        forged.x86_group_ids[forged.x86_group_count++] =
            CDISASM_X86_GROUP_AVX2;
        expect_format_rejected(&forged);
        forged = instruction;
        forged.mask_mode = CDISASM_X86_MASK_MERGE;
        forged.mask_reg = CDISASM_X86_REG_K1;
        expect_format_rejected(&forged);
        forged = instruction;
        forged.opcode[0].access = CDISASM_OPERAND_ACCESS_READ;
        expect_format_rejected(&forged);
        forged = instruction;
        ++forged.opcode[0].size;
        expect_format_rejected(&forged);
        forged = instruction;
        ++forged.opcode[1].reg;
        expect_format_rejected(&forged);
        forged = instruction;
        forged.opcode[1].size = 32u;
        expect_format_rejected(&forged);
        forged = instruction;
        forged.opcode[2].imm = UINT64_C(256);
        expect_format_rejected(&forged);
        forged = instruction;
        forged.opcode[2].access = CDISASM_OPERAND_ACCESS_WRITE;
        expect_format_rejected(&forged);
        forged = instruction;
        forged.encoding.modrm ^= map1 ? UINT8_C(8) : UINT8_C(1);
        expect_format_rejected(&forged);
        forged = instruction;
        forged.encoding.immediate_count = 0u;
        expect_format_rejected(&forged);
        forged = instruction;
        forged.encoding.prefix_size = evex ? 3u : 1u;
        expect_format_rejected(&forged);
        if (!evex && map1 && instruction.encoding.prefix_size == 2u) {
            forged = instruction;
            forged.opcode[1].reg = (cdisasm_x86_reg_id)(
                forged.opcode[1].reg + 8u);
            forged.x86_group_ids[1] = forged.x86_group_ids[0];
            forged.x86_group_ids[0] = CDISASM_X86_GROUP_AMD64;
            forged.x86_group_count = 2u;
            expect_format_rejected(&forged);
        }
        if ((instruction.opcode_flags
                & (CDISASM_PREFIX_ADDRESS_SIZE
                    | CDISASM_PREFIX_SEGMENT)) == 0u) {
            const int ambiguous_low_map1_c4 = !evex && map1
                && instruction.encoding.prefix_size == 3u
                && instruction.opcode[1].reg <= CDISASM_X86_REG_XMM7;

            if (!ambiguous_low_map1_c4) {
                forged = instruction;
                forged.opcode_flags |= CDISASM_PREFIX_ADDRESS_SIZE;
                expect_format_rejected(&forged);
                forged = instruction;
                forged.opcode_flags |=
                    CDISASM_PREFIX_ADDRESS_SIZE | CDISASM_PREFIX_SEGMENT;
                ++forged.opcode_size;
                ++forged.encoding.prefix_size;
                ++forged.encoding.opcode_offset;
                ++forged.encoding.modrm_offset;
                ++forged.encoding.immediate_offset[0];
                expect_format_rejected(&forged);
            }
            forged = instruction;
            ++forged.opcode_size;
            ++forged.encoding.prefix_size;
            ++forged.encoding.opcode_offset;
            ++forged.encoding.modrm_offset;
            ++forged.encoding.immediate_offset[0];
            if (!evex && map1
                && instruction.encoding.prefix_size == 2u) {
                ++forged.opcode_size;
                ++forged.encoding.prefix_size;
                ++forged.encoding.opcode_offset;
                ++forged.encoding.modrm_offset;
                ++forged.encoding.immediate_offset[0];
            }
            expect_format_rejected(&forged);
        }
    }
}
#else
static void test_formatting_and_schema(void)
{
}
#endif

int main(void)
{
    test_complete_c4_partition();
    test_complete_c5_partition();
    test_forms_gates_and_aliases();
    test_controls_and_precedence();
    test_legacy_and_evex_isolation();
    test_formatting_and_schema();

    if (failures != 0) {
        fprintf(stderr, "%d VPEXTR test(s) failed\n", failures);
        return 1;
    }
    return 0;
}
