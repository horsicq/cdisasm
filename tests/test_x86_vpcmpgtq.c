...
static cdisasm_x86_form_id expected_form(
    ...
    int register_form)
{
    return (cdisasm_x86_form_id)(UINT16_C(6502) + 2u * l
        + (register_form ? 1u : 0u));
}
    ...
    unsigned int source2)
{
    ...
    EXPECT(instruction->name_id == CDISASM_X86_NAME_VPCMPGTQ);
...
{
#if USE_EXTRA_OPCODES
    static const uint8_t xmm_reg[] = {0xc4,0xe2,0x69,0x37,0xcb};
    static const uint8_t xmm_reg_w1[] = {0xc4,0xe2,0xe9,0x37,0xcb};
    static const uint8_t ymm_reg[] = {0xc4,0xe2,0x6d,0x37,0xcb};
    static const uint8_t high_reg[] = {0xc4,0x02,0x09,0x37,0xfd};
    static const uint8_t high_memory[] =
        {0xc4,0x22,0x69,0x37,0x44,0x58,0x20};
    static const uint8_t ymm_memory[] =
        {0xc4,0x62,0x6d,0x37,0x7c,0x8b,0xf0};
    static const uint8_t rip_relative[] =
        {0xc4,0xe2,0x69,0x37,0x05,0x78,0x56,0x34,0x12};
    static const uint8_t address_override[] =
        {0x67,0xc4,0xe2,0x69,0x37,0x00};
    static const uint8_t segment_override[] =
        {0x64,0xc4,0xe2,0x69,0x37,0x00};
    static const uint8_t duplicate_address[] =
        {0x67,0x67,0xc4,0xe2,0x69,0x37,0x00};
    static const uint8_t duplicate_segment[] =
        {0x64,0x65,0xc4,0xe2,0x69,0x37,0x00};
    cdisasm_x86_decode_flags flags =
    ...
    check_vpcmpgtq(&instruction, decoded_size, CDISASM_MODE_64,
        0u, 1, 1u, 2u, 3u);
    ...
    for (mode_index = 0u; mode_index < 2u; ++mode_index) {
        ...
        static const uint8_t b_vvvv_alias[] =
            {0xc4,0xc2,0x29,0x37,0xfa};
        static const uint8_t canonical[] =
            {0xc4,0xe2,0x69,0x37,0xfa};
        ...
        check_vpcmpgtq(&instruction, decoded_size, modes[mode_index],
            0u, 1, 7u, 2u, 2u);
        instruction = decode(CDISASM_CPU_X86, modes[mode_index],
...
{
    static const uint8_t xmm[] = {0xc4,0xe2,0x69,0x37,0xcb};
    static const uint8_t ymm[] = {0xc4,0xe2,0x6d,0x37,0xcb};
    ...
    expect_error("VPCMPGTQ XMM needs AVX", CDISASM_CPU_X86,
    ...
    for (cpu_id = CDISASM_CPU_FIRST; cpu_id <= CDISASM_CPU_LAST; ++cpu_id) {
        for (mode_index = 0u; mode_index < 3u; ++mode_index) {
            ...
            if (has_avx) {
                ...
                check_vpcmpgtq(&instruction, decoded_size, mode,
                    0u, 1, 1u, 2u, 3u);
            } else {
...
#else
    expect_error("VPCMPGTQ XMM extras off", CDISASM_CPU_X86,
        CDISASM_MODE_64, xmm, sizeof(xmm), NULL,
        CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
    expect_error("VPCMPGTQ YMM extras off", CDISASM_CPU_X86,
...
{
    static const uint8_t complete[] = {0xc4,0xe2,0x69,0x37,0xcb};
    static const uint8_t legacy_prefixes[5] = {0x66,0xf2,0xf3,0xf0,0x48};
    size_t index;

    for (index = 1u; index < sizeof(complete); ++index) {
        expect_error("truncated VPCMPGTQ", CDISASM_CPU_X86,
            CDISASM_MODE_64, complete, index, NULL,
            CDISASM_STATUS_TRUNCATED);
    }
    expect_error("reserved pp missing SIB", CDISASM_CPU_X86,
        CDISASM_MODE_64,
        (const uint8_t[]){0xc4,0xe2,0x68,0x37,0x04}, 5u, NULL,
        CDISASM_STATUS_TRUNCATED);
    expect_error("reserved pp complete SIB", CDISASM_CPU_X86,
        CDISASM_MODE_64,
        (const uint8_t[]){0xc4,0xe2,0x68,0x37,0x04,0x24}, 6u, NULL,
        CDISASM_STATUS_INVALID_INSTRUCTION);
    expect_error("reserved pp missing disp8", CDISASM_CPU_X86,
        CDISASM_MODE_64,
        (const uint8_t[]){0xc4,0xe2,0x6b,0x37,0x44,0x24}, 6u, NULL,
        CDISASM_STATUS_TRUNCATED);
    expect_error("reserved pp complete disp8", CDISASM_CPU_X86,
        CDISASM_MODE_64,
        (const uint8_t[]){0xc4,0xe2,0x6b,0x37,0x44,0x24,0x7f}, 7u, NULL,
        CDISASM_STATUS_INVALID_INSTRUCTION);

    for (index = 0u; index < sizeof(legacy_prefixes); ++index) {
        const uint8_t prefixed[] = {
            legacy_prefixes[index],0xc4,0xe2,0x69,0x37,0x04,0x24
        };

        expect_error("prefixed VPCMPGTQ missing SIB", CDISASM_CPU_X86,
            CDISASM_MODE_64, prefixed, sizeof(prefixed) - 1u, NULL,
            CDISASM_STATUS_TRUNCATED);
        expect_error("prefixed VPCMPGTQ complete", CDISASM_CPU_X86,
            CDISASM_MODE_64, prefixed, sizeof(prefixed), NULL,
            CDISASM_STATUS_INVALID_INSTRUCTION);
    }
    expect_error("C5 cannot encode map 2 VPCMPGTQ", CDISASM_CPU_X86,
        CDISASM_MODE_64,
        (const uint8_t[]){0xc5,0xf1,0x37,0xd3}, 4u, NULL,
        CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);
    expect_error("C4 map 1 is not VPCMPGTQ", CDISASM_CPU_X86,
        CDISASM_MODE_64,
        (const uint8_t[]){0xc4,0xe1,0x71,0x37,0xd3}, 5u, NULL,
        CDISASM_STATUS_UNSUPPORTED_INSTRUCTION);

#if USE_EXTRA_OPCODES
    {
        static const uint8_t legacy[] = {0x66,0x0f,0x38,0x37,0xcb};
        static const uint8_t evex[] = {0x62,0xf2,0xed,0x08,0x37,0xcb};
        ...
        static const uint8_t r_collision[] =
            {0xc4,0x62,0x69,0x37,0xcb,0,0,0,0};
        static const uint8_t x_collision[] =
            {0xc4,0xa2,0x69,0x37,0xcb,0,0,0,0};
        ...
        EXPECT(instruction.name_id == CDISASM_X86_NAME_PCMPGTQ);
        EXPECT((instruction.opcode_flags
        ...
        EXPECT(instruction.name_id == CDISASM_X86_NAME_VPCMPGTQ);
        EXPECT(instruction.form_id >= UINT16_C(6496)
...
static void test_formatting_and_schema(void)
{
    ...
    static const struct format_case {
        ...
        {{0xc4,0xe2,0x69,0x37,0xcb,0,0,0},5u,
            "vpcmpgtq xmm1, xmm2, xmm3",
            "vpcmpgtq %xmm3, %xmm2, %xmm1"},
        {{0xc4,0xe2,0xed,0x37,0xcb,0,0,0},5u,
            "vpcmpgtq ymm1, ymm2, ymm3",
            "vpcmpgtq %ymm3, %ymm2, %ymm1"},
        {{0xc4,0x22,0x69,0x37,0x44,0x58,0x20,0},7u,
            "vpcmpgtq xmm8, xmm2, xmmword ptr [rax + r11*2 + 0x20]",
            "vpcmpgtq 0x20(%rax,%r11,2), %xmm2, %xmm8"},
        {{0xc4,0x62,0x6d,0x37,0x7c,0x8b,0xf0,0},7u,
            "vpcmpgtq ymm15, ymm2, ymmword ptr [rbx + rcx*4 - 0x10]",
            "vpcmpgtq -0x10(%rbx,%rcx,4), %ymm2, %ymm15"},
        {{0x62,0xf2,0xed,0x08,0x37,0x00,0,0},6u,
            "vpcmpgtq k0, xmm2, xmmword ptr [rax]",
            "vpcmpgtq (%rax), %xmm2, %k0"},
        {{0x62,0xf2,0xed,0x08,0x37,0xcb,0,0},6u,
            "vpcmpgtq k1, xmm2, xmm3",
            "vpcmpgtq %xmm3, %xmm2, %k1"},
        {{0x62,0xf2,0xed,0x28,0x37,0x00,0,0},6u,
            "vpcmpgtq k0, ymm2, ymmword ptr [rax]",
            "vpcmpgtq (%rax), %ymm2, %k0"},
        {{0x62,0xf2,0xed,0x28,0x37,0xcb,0,0},6u,
            "vpcmpgtq k1, ymm2, ymm3",
            "vpcmpgtq %ymm3, %ymm2, %k1"},
        {{0x62,0xf2,0xed,0x48,0x37,0x00,0,0},6u,
            "vpcmpgtq k0, zmm2, zmmword ptr [rax]",
            "vpcmpgtq (%rax), %zmm2, %k0"},
        {{0x62,0xf2,0xed,0x48,0x37,0xcb,0,0},6u,
            "vpcmpgtq k1, zmm2, zmm3",
            "vpcmpgtq %zmm3, %zmm2, %k1"},
        {{0x62,0xf2,0xed,0x1a,0x37,0x00,0,0},6u,
            "vpcmpgtq k0 {k2}, xmm2, qword ptr [rax]{1to2}",
            "vpcmpgtq (%rax){1to2}, %xmm2, %k0{%k2}"}
    };
    cdisasm_x86_decode_flags flags =
        cpu_flags(CDISASM_CPU_X86, CDISASM_MODE_64);
    size_t index;
    ...
    for (index = 0u; index < sizeof(cases) / sizeof(cases[0]); ++index) {
        ...
        if (index >= 4u) {
            ...
            EXPECT(instruction.form_id == (cdisasm_x86_form_id)(
                UINT16_C(6496) + (evex_case < 6u ? evex_case : 0u)));
            EXPECT(instruction.opcode_flags
                == (CDISASM_PREFIX_EVEX
                    | CDISASM_X86_INSTRUCTION_FLAG_GENERATED_FALLBACK));
            EXPECT(instruction.operand_count == 3u);
    ...
    {
        static const uint8_t code[] = {0xc4,0xe2,0x69,0x37,0xcb};
...

    {
        static const uint8_t memory_code[] = {
            0x64,0x67,0xc4,0xe2,0x71,0x37,0x44,0x24,0x20};
        cdisasm_instruction valid;
        cdisasm_instruction forged;
        uint32_t decoded_size;
        ...
        EXPECT(valid.form_id == UINT16_C(6502));
        forged = valid;
...

    {
        ...
        EXPECT(valid.form_id == UINT16_C(6497));
        forged = valid;
        forged.name_id = CDISASM_X86_NAME_ADD;
        expect_forged_format_rejected(&forged);
        forged = valid;
        forged.form_id = UINT16_C(6496);
        expect_forged_format_rejected(&forged);
        forged = valid;
        forged.form_id = UINT16_C(6502);
        expect_forged_format_rejected(&forged);
...

    {
        static const uint8_t evex_memory_code[] = {
            0x64,0x62,0xf2,0xed,0x1a,0x37,0x00};
        cdisasm_instruction valid;
        cdisasm_instruction forged;
        uint32_t decoded_size;
        ...
        EXPECT(valid.form_id == UINT16_C(6496));
        EXPECT(valid.opcode[2].size == 8u);
...
int main(void)
{
    ...
    if (failures != 0) {
        fprintf(stderr, "x86 VPCMPGTQ tests: %d failure(s)\n", failures);
        return 1;
    }
    puts("x86 VPCMPGTQ tests passed");
    return 0;
}
