...
typedef struct conversion_form {
    uint32_t base;
    uint8_t destination_size;
    uint8_t source_size;
    cdisasm_arm_form_id signed_form_id;
    cdisasm_arm_form_id unsigned_form_id;
} conversion_form;

static const conversion_form forms[4] = {
    { UINT32_C(0x6594a000), 4u, 4u, UINT16_C(3128), UINT16_C(3135) },
    { UINT32_C(0x65d4a000), 4u, 8u, UINT16_C(3130), UINT16_C(3137) },
    { UINT32_C(0x65d0a000), 8u, 4u, UINT16_C(3129), UINT16_C(3136) },
    { UINT32_C(0x65d6a000), 8u, 8u, UINT16_C(3131), UINT16_C(3138) }
};
...
static uint32_t conversion_word(
    unsigned form, unsigned is_unsigned, unsigned pg, unsigned zn,
    unsigned zd)
{
    return forms[form].base
        | ((uint32_t)is_unsigned << 16)
        | ((uint32_t)pg << 10)
        | ((uint32_t)zn << 5)
        | (uint32_t)zd;
}

static int is_conversion_word(uint32_t word)
{
    unsigned index;

    for (index = 0u; index < 4u; ++index) {
        if ((word & UINT32_C(0xfffee000)) == forms[index].base) {
            return 1;
        }
    }
    return 0;
...
static int is_existing_fp16_conversion_word(uint32_t word)
{
    return (word & UINT32_C(0xfff8e000)) == UINT32_C(0x6550a000)
        && ((word >> 16) & 7u) >= 2u;
}
    ...
    cdisasm_arm_instruction *instruction)
{
    ...
    return cdisasm_arm_decode(cpu_id, mode, bytes, code_size,
        UINT64_C(0x125000), options, instruction);
}
...
static void build_expected_instruction(
    uint32_t word, unsigned form, unsigned is_unsigned, unsigned pg,
    unsigned zn, unsigned zd, cdisasm_arm_instruction *expected)
{
    memset(expected, 0, sizeof(*expected));
    expected->address = UINT64_C(0x125000);
    ...
    expected->name_id = is_unsigned != 0u
    ...
    expected->form_id = is_unsigned != 0u
        ? forms[form].unsigned_form_id : forms[form].signed_form_id;
    ...
    expected->operand[0].extend_type =
        (cdisasm_arm_extend_type)forms[form].destination_size;
    ...
    expected->operand[1].extend_type =
        (cdisasm_arm_extend_type)forms[form].source_size;
...
static void test_exhaustive_exact_domains(void)
{
        ...
        for (is_unsigned = 0u; is_unsigned < 2u; ++is_unsigned) {
            ...
            for (pg = 0u; pg < 8u; ++pg) {
                ...
                for (zn = 0u; zn < 32u; ++zn) {
                    ...
                    for (zd = 0u; zd < 32u; ++zd) {
                        cdisasm_arm_instruction instruction;
                        uint32_t word = conversion_word(
                            form, is_unsigned, pg, zn, zd);
                        uint32_t decoded;

                        domain_expect(is_conversion_word(word), word,
                            "word escaped exact classifier union");
...
#if USE_EXTRA_OPCODES
                        {
                            ...
                            build_expected_instruction(
                                word, form, is_unsigned, pg, zn, zd,
                                &expected);
                            ...
                            domain_expect(memcmp(
                                ...
                                sizeof(expected)) == 0,
                                word, "conversion metadata mismatch");
                        }
#else
...
{
    const uint32_t word = conversion_word(1u, 1u, 3u, 13u, 7u);
    uint32_t cpu_value;
...
static void test_fixed_bit_neighbors(void)
{
    ...
    for (form = 0u; form < 4u; ++form) {
        const uint32_t canonical = conversion_word(form, 0u, 3u, 13u, 7u);
        unsigned bit;
        ...
        for (bit = 0u; bit < 32u; ++bit) {
            ...
            if (is_conversion_word(neighbor)
                || is_existing_fp16_conversion_word(neighbor)) {
                continue;
            }
            memset(&instruction, 0xa5, sizeof(instruction));
            decoded = decode_word(
...
{
    const uint32_t word = conversion_word(3u, 1u, 7u, 31u, 31u);
    ...
    decoded = cdisasm_arm_decode(
        ...
        bytes, sizeof(bytes), UINT64_C(0x125000),
...
{
    static const char *const expected[4][2] = {
        { "scvtf z7.s, p3/m, z13.s", "ucvtf z7.s, p3/m, z13.s" },
        { "scvtf z7.s, p3/m, z13.d", "ucvtf z7.s, p3/m, z13.d" },
        { "scvtf z7.d, p3/m, z13.s", "ucvtf z7.d, p3/m, z13.s" },
        { "scvtf z7.d, p3/m, z13.d", "ucvtf z7.d, p3/m, z13.d" }
    };
    unsigned form;
    ...
    for (form = 0u; form < 4u; ++form) {
        ...
        for (is_unsigned = 0u; is_unsigned < 2u; ++is_unsigned) {
            expect_format(
                conversion_word(form, is_unsigned, 3u, 13u, 7u),
                expected[form][is_unsigned]);
        }
    }

    {
        ...
        memset(&instruction, 0xa5, sizeof(instruction));
        EXPECT(decode_word(
            conversion_word(3u, 1u, 7u, 31u, 31u),
...
int main(void)
{
    ...
    if (failures != 0) {
        fprintf(stderr,
            "%d ARM SVE integer-to-float test(s) failed\n", failures);
        return 1;
    }
    printf("ARM SVE integer-to-float tests passed "
           "(USE_EXTRA_OPCODES=%d, exact envelopes=4, allocated=65536)\n",
           USE_EXTRA_OPCODES);
    return 0;
}
