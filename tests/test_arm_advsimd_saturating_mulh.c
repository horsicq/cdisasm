...
typedef struct mulh_descriptor {
    uint32_t value;
...
static const mulh_descriptor descriptors[] = {
    { UINT32_C(0x0e20b400), CDISASM_ARM_NAME_SQDMULH,
      UINT16_C(6136), "sqdmulh" },
    { UINT32_C(0x2e20b400), CDISASM_ARM_NAME_SQRDMULH,
      UINT16_C(6178), "sqrdmulh" }
};

_Static_assert(CDISASM_ARM_NAME_SQDMULH == UINT16_C(1483)
                   && CDISASM_ARM_NAME_SQRDMULH == UINT16_C(1495),
               "saturating multiply-high mnemonic IDs changed");
_Static_assert(CDISASM_ARM_FORM_AARCHMRS_2026_03_LAST >= UINT16_C(6178),
               "saturating multiply-high form IDs unavailable");
...
static uint32_t make_word(const mulh_descriptor *descriptor,
                            ...
                            cdisasm_arm_instruction *instruction)
{
    ...
    return cdisasm_arm_decode(cpu, mode, bytes, code_size,
        UINT64_C(0x162000), options, instruction);
}
...
static int metadata_matches(const cdisasm_arm_instruction *instruction,
                            uint32_t word,
                            const mulh_descriptor *descriptor,
                            unsigned q, unsigned size_code,
                            unsigned rm, unsigned rn, unsigned rd)
{
    ...
    return instruction->last_error_id == CDISASM_STATUS_OK
        ...
        && instruction->address == UINT64_C(0x162000)
...
{
    uint32_t partitions[2][2][2] = {{{ 0u }}};
    uint32_t allocated = 0u;
    uint32_t reserved = 0u;
    size_t descriptor_index;

    for (descriptor_index = 0u; descriptor_index < 2u;
         ++descriptor_index) {
        const mulh_descriptor *descriptor = &descriptors[descriptor_index];
        unsigned q;
            ...
            for (size_code = 0u; size_code < 4u; ++size_code) {
                ...
                for (rm = 0u; rm < 32u; ++rm) {
                    ...
                    for (rn = 0u; rn < 32u; ++rn) {
                        ...
                        for (rd = 0u; rd < 32u; ++rd) {
                            cdisasm_arm_instruction instruction;
                            uint32_t word = make_word(descriptor, q,
                                size_code, rm, rn, rd);
                            uint32_t decoded;
...
static void test_profiles_and_neighbors(void)
{
    static const uint32_t adjacent[] = {
        UINT32_C(0x0e20b000), UINT32_C(0x0e20b800),
        UINT32_C(0x0e20bc00), UINT32_C(0x2e20b000),
        UINT32_C(0x2e20b800), UINT32_C(0x2e20bc00)
    };
    size_t descriptor_index;
    ...
    for (descriptor_index = 0u; descriptor_index < 2u;
         ++descriptor_index) {
        const mulh_descriptor *descriptor = &descriptors[descriptor_index];
        uint32_t word = make_word(descriptor, 1u, 2u, 29u, 13u, 7u);
        unsigned bit;
                            ...
                            uint32_t reserved_size3)
{
    ...
    EXPECT(cdisasm_arm_decode(CDISASM_ARM_CPU_ANY,
        CDISASM_ARM_MODE_A64, bytes, sizeof(bytes), UINT64_C(0x162000),
    ...
    EXPECT(cdisasm_decode(CDISASM_ARM_CPU_ANY, CDISASM_ARM_MODE_A64,
        bytes, sizeof(bytes), UINT64_C(0x162000),
...
static void test_transport_and_boundaries(void)
{
    ...
    for (descriptor_index = 0u; descriptor_index < 2u;
        ...
        check_transport(make_word(descriptor, descriptor_index & 1u,
                                  1u + (unsigned)(descriptor_index & 1u),
                                  29u, 13u, 7u),
                        make_word(descriptor, descriptor_index & 1u, 0u,
                                  29u, 13u, 7u),
                        make_word(descriptor, descriptor_index & 1u, 3u,
                                  29u, 13u, 7u));
    }
    ...
    memset(&instruction, 0xa5, sizeof(instruction));
    EXPECT(decode_word(make_word(&descriptors[0], 0u, 2u, 2u, 1u, 0u),
...
static void expect_opaque_format(cdisasm_arm_instruction *instruction,
                                 const char *expected)
{
    char text[160];
    ...
    if (length != strlen(expected) || strcmp(text, expected) != 0) {
        fprintf(stderr, "opaque format mismatch: expected '%s', got '%s'\n",
                expected, text);
    }
...
{
    static const char element_suffix[2] = { 'h', 's' };
    cdisasm_arm_instruction instruction;
    ...
    memset(&instruction, 0xa5, sizeof(instruction));
    EXPECT(decode_word(make_word(&descriptors[1], 1u, 2u,
    ...
    EXPECT(cdisasm_arm_format(&instruction,
        ...
        text, sizeof(text))
        == strlen("SQRDMULH v31.4s, v30.4s, v29.4s"));
    EXPECT(strcmp(text, "SQRDMULH v31.4s, v30.4s, v29.4s") == 0);
...
#define REJECT_MUTATION(statement)                                          \
    ...
    REJECT_MUTATION(forged.form_id = UINT16_C(6136));
    REJECT_MUTATION(forged.name_id = CDISASM_ARM_NAME_SQDMULH);
    REJECT_MUTATION(forged.raw_instruction = make_word(
        &descriptors[1], 0u, 2u, 29u, 30u, 31u));
    REJECT_MUTATION(forged.raw_instruction = make_word(
        &descriptors[1], 1u, 0u, 29u, 30u, 31u));
    REJECT_MUTATION(forged.raw_instruction = make_word(
        &descriptors[1], 1u, 3u, 29u, 30u, 31u));
    ...
    /* Raw, form, and name claims independently enter the exact validator. */
    ...
    memset(&forged, 0, sizeof(forged));
    forged.name_id = CDISASM_ARM_NAME_SQDMULH;
    forged.raw_instruction = UINT32_C(0xd503201f);
    forged.opcode_size = 4u;
    forged.isa_id = CDISASM_ARM_ISA_A64;
    forged.condition = CDISASM_ARM_CONDITION_AL;
    ...
    /* Structured SVE siblings remain independently owned. */
    expect_format(UINT32_C(0x042b70c1),
                  "sqdmulh z1.b, z6.b, z11.b");
    expect_format(UINT32_C(0x042b74c1),
                  "sqrdmulh z1.b, z6.b, z11.b");
    memset(&instruction, 0xa5, sizeof(instruction));
    EXPECT(decode_word(UINT32_C(0x042b70c1), CDISASM_ARM_CPU_ANY,
        CDISASM_ARM_MODE_A64, 4u, CDISASM_ARM_DECODE_OPTION_NONE,
        &instruction) == 4u);
    forged = instruction;
    forged.name_id = CDISASM_ARM_NAME_SQRDMULH;
    reject_forgery(&forged, "SVE sibling cross-name graft");
    forged = instruction;
    forged.form_id = UINT16_C(6136);
    reject_forgery(&forged, "SVE raw with fixed-vector form");

    /* Exact generated scalar siblings retain their existing renderer. */
    init_opaque(&instruction, CDISASM_ARM_NAME_SQDMULH,
                UINT16_C(5832), UINT32_C(0x5ea2b420));
    expect_opaque_format(&instruction, "sqdmulh s0, s1, s2");
    forged = instruction;
    forged.raw_instruction = UINT32_C(0xd503201f);
    ...
    init_opaque(&instruction, CDISASM_ARM_NAME_SQRDMULH,
                UINT16_C(5847), UINT32_C(0x7ea2b420));
    expect_opaque_format(&instruction, "sqrdmulh s0, s1, s2");
    forged = instruction;
    forged.name_id = CDISASM_ARM_NAME_SQDMULH;
    reject_forgery(&forged, "scalar sibling cross-name graft");

    /* Split-index recipes remain safely opaque and cannot be claimed as
     * this fixed-vector family. */
    init_opaque(&instruction, CDISASM_ARM_NAME_SQDMULH,
                UINT16_C(6250), UINT32_C(0x0f72c020));
    ...
    init_opaque(&instruction, CDISASM_ARM_NAME_SQRDMULH,
                UINT16_C(5881), UINT32_C(0x5f72d020));
    reject_forgery(&instruction, "opaque scalar by-element sibling");

    memset(&instruction, 0xa5, sizeof(instruction));
    EXPECT(decode_word(UINT32_C(0xc120a400), CDISASM_ARM_CPU_ANY,
        CDISASM_ARM_MODE_A64, 4u, CDISASM_ARM_DECODE_OPTION_NONE,
        &instruction) == 4u);
    expect_opaque_format(&instruction,
        "sqdmulh {z0.b, z1.b}, {z0.b, z1.b}, z0.b");
    forged = instruction;
    forged.form_id = UINT16_C(6136);
    reject_forgery(&forged, "SME raw with fixed-vector form");
...
int main(void)
{
    ...
    if (failures != 0) {
        fprintf(stderr, "%d saturating multiply-high test(s) failed\n",
                failures);
        return 1;
    }
    printf("ARM Advanced SIMD SQDMULH/SQRDMULH tests passed "
           "(USE_EXTRA_OPCODES=%d, allocated=262144, reserved=262144)\n",
           USE_EXTRA_OPCODES);
    return 0;
}
