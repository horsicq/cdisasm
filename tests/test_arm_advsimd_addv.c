...
#define ADDV_MASK UINT32_C(0xbf3ffc00)
#define ADDV_VALUE UINT32_C(0x0e31b800)
#define ADDV_FORM UINT16_C(6077)

_Static_assert(CDISASM_ARM_NAME_ADDV == UINT16_C(526),
               "ADDV mnemonic ID changed");
_Static_assert(CDISASM_ARM_FORM_AARCHMRS_2026_03_LAST >= ADDV_FORM,
               "Advanced SIMD ADDV form ID unavailable");
...
{
    return ADDV_VALUE | ((uint32_t)q << 30)
                            ...
                            cdisasm_arm_instruction *instruction)
{
    ...
    return cdisasm_arm_decode(cpu, mode, bytes, code_size,
        UINT64_C(0x16b000), options, instruction);
}
...
static void test_exhaustive_envelope(void)
{
    ...
    for (q = 0u; q < 2u; ++q) {
        ...
        for (size_code = 0u; size_code < 4u; ++size_code) {
            ...
            for (rn = 0u; rn < 32u; ++rn) {
                ...
                for (rd = 0u; rd < 32u; ++rd) {
                    cdisasm_arm_instruction instruction;
                    uint32_t word = make_word(q, size_code, rn, rd);
                    uint32_t decoded;

                    ++partitions[q][size_code][is_allocated];
                    if (is_allocated) {
                        ++allocated;
                    } else {
                        ++reserved;
                    }
                    EXPECT((word & ADDV_MASK) == ADDV_VALUE);
...
#if USE_EXTRA_OPCODES
                        EXPECT(decoded == 4u);
                        EXPECT(metadata_matches(&instruction, word, q,
                                                size_code, rn, rd));
#else
                        EXPECT(decoded == 0u);
    ...
    EXPECT(allocated == UINT32_C(5120));
    ...
    for (q = 0u; q < 2u; ++q) {
        ...
        for (size_code = 0u; size_code < 4u; ++size_code) {
            ...
            EXPECT(partitions[q][size_code][is_allocated]
                == UINT32_C(1024));
            EXPECT(partitions[q][size_code][!is_allocated] == 0u);
        }
    }
}
...
static void test_profiles_and_collision_boundaries(void)
{
    ...
    for (bit = 0u; bit < 32u; ++bit) {
        ...
        if ((ADDV_MASK & (UINT32_C(1) << bit)) == 0u) {
            continue;
        }
        memset(&instruction, 0xa5, sizeof(instruction));
...
static void check_transport(uint32_t word)
{
    ...
    EXPECT(cdisasm_arm_decode(CDISASM_ARM_CPU_ANY,
        CDISASM_ARM_MODE_A64, bytes, sizeof(bytes), UINT64_C(0x16b000),
...
{
    cdisasm_arm_instruction instruction;
    uint32_t allocated = make_word(1u, 2u, 13u, 7u);
    uint32_t reserved_2s = make_word(0u, 2u, 13u, 7u);
    uint32_t reserved_1d = make_word(0u, 3u, 13u, 7u);
    uint32_t reserved_2d = make_word(1u, 3u, 13u, 7u);

    check_transport(make_word(0u, 0u, 0u, 0u));
...
static void test_formatter_and_schema(void)
{
    ...
    expect_format(make_word(0u, 0u, 0u, 0u),
                  "addv b0, v0.8b");
    expect_format(make_word(1u, 0u, 31u, 31u),
                  "addv b31, v31.16b");
    expect_format(make_word(0u, 1u, 13u, 7u),
                  "addv h7, v13.4h");
    expect_format(make_word(1u, 1u, 13u, 7u),
                  "addv h7, v13.8h");
    expect_format(make_word(1u, 2u, 13u, 7u),
                  "addv s7, v13.4s");

    memset(&instruction, 0xa5, sizeof(instruction));
    EXPECT(decode_word(make_word(1u, 2u, 30u, 31u),
        CDISASM_ARM_CPU_ANY, CDISASM_ARM_MODE_A64, 4u,
        CDISASM_ARM_DECODE_OPTION_NONE, &instruction) == 4u);
    EXPECT(cdisasm_arm_format(&instruction,
        CDISASM_FORMAT_SYNTAX_ARM_CANONICAL
            | CDISASM_FORMAT_UPPERCASE_OPCODE,
        text, sizeof(text)) == strlen("ADDV s31, v30.4s"));
    EXPECT(strcmp(text, "ADDV s31, v30.4s") == 0);
...
#define REJECT_MUTATION(statement)                                          \
    ...
    REJECT_MUTATION(forged.form_id = UINT16_C(6076));
    REJECT_MUTATION(forged.name_id = CDISASM_ARM_NAME_ADDP);
    ...
    REJECT_MUTATION(forged.operand[0].reg = CDISASM_ARM_REG_S30);
    REJECT_MUTATION(forged.operand[0].size = 2u);
    ...
    forged.raw_instruction = make_word(1u, 0u, 1u, 0u);
    reject_forgery(&forged, "raw-only claim");

    memset(&forged, 0, sizeof(forged));
    forged.name_id = CDISASM_ARM_NAME_ADDV;
    forged.raw_instruction = UINT32_C(0xd503201f);
    forged.opcode_size = 4u;
    forged.isa_id = CDISASM_ARM_ISA_A64;
    forged.condition = CDISASM_ARM_CONDITION_AL;
...
int main(void)
{
    ...
    if (failures != 0) {
        fprintf(stderr, "%d Advanced SIMD ADDV test(s) failed\n",
                failures);
        return 1;
    }
    printf("ARM Advanced SIMD ADDV tests passed "
           "(USE_EXTRA_OPCODES=%d, allocated=5120, reserved=3072)\n",
           USE_EXTRA_OPCODES);
    return 0;
}
