...
typedef struct widening_descriptor {
    uint32_t value;
...
/* Pinned AARCHMRS forms SADDL/SADDW/SSUBL/SSUBW_asimddiff and their
 * unsigned counterparts.  LLVM 21 independently emits the same fixed
 * words, size arrangements, and Q-selected mnemonic suffix. */
static const widening_descriptor descriptors[] = {
    { UINT32_C(0x0e200000), CDISASM_ARM_NAME_SADDL,
      UINT16_C(6089), "saddl", 0u },
    { UINT32_C(0x0e201000), CDISASM_ARM_NAME_SADDW,
      UINT16_C(6090), "saddw", 1u },
    { UINT32_C(0x0e202000), CDISASM_ARM_NAME_SSUBL,
      UINT16_C(6091), "ssubl", 0u },
    { UINT32_C(0x0e203000), CDISASM_ARM_NAME_SSUBW,
      UINT16_C(6092), "ssubw", 1u },
    { UINT32_C(0x2e200000), CDISASM_ARM_NAME_UADDL,
      UINT16_C(6104), "uaddl", 0u },
    { UINT32_C(0x2e201000), CDISASM_ARM_NAME_UADDW,
      UINT16_C(6105), "uaddw", 1u },
    { UINT32_C(0x2e202000), CDISASM_ARM_NAME_USUBL,
      UINT16_C(6106), "usubl", 0u },
    { UINT32_C(0x2e203000), CDISASM_ARM_NAME_USUBW,
      UINT16_C(6107), "usubw", 1u }
};

_Static_assert(CDISASM_ARM_NAME_SADDL == UINT16_C(1307),
               "established SADDL ID changed");
_Static_assert(CDISASM_ARM_NAME_SADDW == UINT16_C(1314),
               "established SADDW ID changed");
_Static_assert(CDISASM_ARM_NAME_SSUBL == UINT16_C(1541),
               "established SSUBL ID changed");
_Static_assert(CDISASM_ARM_NAME_SSUBW == UINT16_C(1546),
               "established SSUBW ID changed");
_Static_assert(CDISASM_ARM_NAME_UADDL == UINT16_C(1738),
               "established UADDL ID changed");
_Static_assert(CDISASM_ARM_NAME_UADDW == UINT16_C(1744),
               "established UADDW ID changed");
_Static_assert(CDISASM_ARM_NAME_USUBL == UINT16_C(1851),
               "established USUBL ID changed");
_Static_assert(CDISASM_ARM_NAME_USUBW == UINT16_C(1854),
               "established USUBW ID changed");
...
static uint32_t widening_word(
    const widening_descriptor *descriptor,
    ...
    unsigned rm, unsigned rn, unsigned rd)
{
    ...
    return instruction->last_error_id == CDISASM_STATUS_OK
        ...
        && vector_matches(&instruction->operand[1], rn,
            descriptor->wide_first != 0u ? 16u : narrow_total_size,
            descriptor->wide_first != 0u
...
{
    uint32_t partition_counts[8][2][2] = {{{ 0u }}};
    ...
    for (descriptor_index = 0u;
         ...
         ++descriptor_index) {
        const widening_descriptor *descriptor = &descriptors[descriptor_index];
        unsigned q;
    ...
    for (descriptor_index = 0u; descriptor_index < 8u;
         ++descriptor_index) {
        unsigned q;
    ...
    EXPECT(allocated == UINT32_C(1572864));
    EXPECT(reserved == UINT32_C(524288));
}
...
static void test_profiles_and_fixed_neighbors(void)
{
    static const uint32_t adjacent_values[] = {
        UINT32_C(0x0e200400), /* SHADD */
        UINT32_C(0x0e200c00), /* SQADD */
        UINT32_C(0x0e204000), /* ADDHN */
        UINT32_C(0x0e205000), /* SABAL */
        UINT32_C(0x0e207000), /* SABDL */
        UINT32_C(0x2e200400), /* UHADD */
        UINT32_C(0x2e200c00), /* UQADD */
...
static void test_endian_dispatch_modes_and_boundaries(void)
{
    ...
    for (descriptor_index = 0u;
        ...
        uint32_t reserved = widening_word(
            &descriptors[descriptor_index], descriptor_index & 1u,
            3u, 29u, 13u, 7u);
    ...
    memset(&instruction, 0xa5, sizeof(instruction));
    EXPECT(decode_word(widening_word(
...
static void test_formatter_and_forgery(void)
{
        ...
        for (q = 0u; q < 2u; ++q) {
            ...
            for (size_code = 0u; size_code < 3u; ++size_code) {
                ...
                unsigned narrow_count = (q != 0u ? 16u : 8u)
                    >> size_code;
                unsigned first_count = descriptors[descriptor_index].wide_first
                    != 0u ? wide_count : narrow_count;
                char first_suffix = descriptors[descriptor_index].wide_first
                    != 0u ? result_suffixes[size_code]
                ...
                int length = snprintf(expected, sizeof(expected),
                    ...
                    q != 0u ? "2" : "", wide_count,
                    result_suffixes[size_code], first_count,
                    first_suffix, narrow_count,
                    source_suffixes[size_code]);

                EXPECT(length > 0 && (size_t)length < sizeof(expected));
                expect_format(widening_word(
    ...
    memset(&instruction, 0xa5, sizeof(instruction));
    EXPECT(decode_word(widening_word(
            &descriptors[7], 1u, 2u, 29u, 30u, 31u),
        CDISASM_ARM_CPU_ANY, CDISASM_ARM_MODE_A64, 4u,
        CDISASM_ARM_DECODE_OPTION_NONE, &instruction) == 4u);
    EXPECT(cdisasm_arm_format(
        ...
        text, sizeof(text))
        == strlen("USUBW2 v31.2d, v30.2d, v29.4s"));
    EXPECT(strcmp(text, "USUBW2 v31.2d, v30.2d, v29.4s") == 0);
...
#define REJECT_MUTATION(statement)                                          \
    ...
    REJECT_MUTATION(forged.form_id = UINT16_C(6106));
    REJECT_MUTATION(forged.name_id = CDISASM_ARM_NAME_USUBL);
    REJECT_MUTATION(forged.raw_instruction = widening_word(
        &descriptors[7], 0u, 2u, 29u, 30u, 31u));
    REJECT_MUTATION(forged.raw_instruction = widening_word(
        &descriptors[7], 1u, 3u, 29u, 30u, 31u));
    ...
    EXPECT(decode_word(UINT32_C(0xd503201f), CDISASM_ARM_CPU_ANY,
        ...
        &instruction) == 4u);
    forged = instruction;
    forged.raw_instruction = widening_word(
        &descriptors[0], 0u, 0u, 29u, 13u, 7u);
    reject_forgery(&forged, "raw-only allocated widening forgery");
    forged = instruction;
    forged.raw_instruction = widening_word(
        &descriptors[4], 1u, 3u, 29u, 13u, 7u);
    reject_forgery(&forged, "raw-only reserved widening forgery");
    ...
    EXPECT(!is_target_form(instruction.form_id));
    forged = instruction;
    forged.form_id = UINT16_C(6089);
    reject_forgery(&forged, "A32 form-only SADDL forgery");
    forged = instruction;
    forged.name_id = CDISASM_ARM_NAME_SADDL;
    reject_forgery(&forged, "A32 name-only SADDL forgery");
    forged.instruction_flags |=
        CDISASM_ARM_INSTRUCTION_FLAG_OPERANDS_OPAQUE;
    reject_forgery(&forged, "A32 opaque name-only SADDL forgery");
    ...
    EXPECT(!is_target_form(instruction.form_id));
    forged = instruction;
    forged.name_id = CDISASM_ARM_NAME_USUBW;
    reject_forgery(&forged, "T32 name-only USUBW forgery");
...
int main(void)
{
    ...
    if (failures != 0) {
        fprintf(stderr,
            "%d ARM Advanced SIMD widening add/sub test(s) failed\n",
            failures);
        return 1;
    }
    printf("ARM Advanced SIMD widening SADDL/SADDW/SSUBL/SSUBW and "
           "UADDL/UADDW/USUBL/USUBW tests passed "
           "(USE_EXTRA_OPCODES=%d, allocated=1572864, reserved=524288)\n",
           USE_EXTRA_OPCODES);
    return 0;
}
